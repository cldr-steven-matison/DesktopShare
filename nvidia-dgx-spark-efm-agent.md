...
>
> **Design record (work-stream G, [#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239)):** MiNiFi **Java** as the runtime, agent class `NvidiaSpark-1`, enrollment only through `generateCommand` with a server-minted `agentIdentifier`, a four-front-door class flow, and a fifth `/metrics` leg matching the fleet's exporter convention. The four target endpoint URLs follow the model lock and `nvidia-dgx-spark-runbook.md` §2. All of this is now built and field-validated on `spark-dd06` (see the status above); the design sections below are the "how it was designed" record behind that build. Jetson latency numbers below are real; DGX Spark latencies are as-built where the two status blocks above cite them.
