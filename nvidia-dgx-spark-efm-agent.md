...
>
> Two items are open, and #239 tracks both. One is the cluster-side Prometheus scrape of `:9936` and `:9835` on WindowsDesktop's stack; the other is end-to-end validation of a §3 use case run from a non-Spark device. Manifests for the scrape, the fleet-board panels, the `:9835` exporter installer and the remote `curl` recipes are in `files/issue-239/`.
>
