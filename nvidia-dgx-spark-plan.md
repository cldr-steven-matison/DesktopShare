...
|---|---|---|---|---|
| A | Capability landscape & model sizing | `nvidia-dgx-spark-landscape.md` | [#232](https://github.com/cldr-steven-matison/DesktopShare/issues/232) | drafted (first package, 10783b1); the planned expansion — MoE-vs-dense, spec-decode, leaderboards, 128 GB co-hosting budget, 3-candidate model lock — **still owed**; in review |
| B | Day-one device runbook | `nvidia-dgx-spark-runbook.md` | [#233](https://github.com/cldr-steven-matison/DesktopShare/issues/233) | drafted (first package); the full unbox → DGX OS → network → containers → k3d → roster → endpoints → hardening expansion **still owed**; in review |
...
| F | k3d + CSO operators (NiFi/Kafka/Flink) on aarch64; GPU-services cutover ladder | `nvidia-dgx-spark-k3d-cso.md` | [#238](https://github.com/cldr-steven-matison/DesktopShare/issues/238) | **written 2026-08-26** — 371 lines, 47 URLs; K's gate answered (arm64-native), fallback table kept as contingency; in review |
| G | EFM agent class `NvidiaSpark-1` + out-of-box use cases | `nvidia-dgx-spark-efm-agent.md` | [#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239) | **written 2026-08-26** — 261 lines, 27 URLs; in review |
| H | Local knowledge base + local agentic validation for Claude Code | `nvidia-dgx-spark-local-kb.md` | [#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240) | **written 2026-08-26** — 282 lines, 39 URLs; in review |
| I | NVIDIA ↔ Cloudera on AWS (CDP Base on AWS + CDP Public Cloud on AWS, Cloudera AI) | `nvidia-dgx-spark-cloudera-aws.md` | [#241](https://github.com/cldr-steven-matison/DesktopShare/issues/241) | **written 2026-08-26** — 346 lines, 45 URLs; in review |
| J | The guide — tracker + 22-chapter skeleton | `Complete Developer Guide for Nvidia Spark with Cloudera.md`, `files/nvidia-spark-guide/` | [#242](https://github.com/cldr-steven-matison/DesktopShare/issues/242) | tracker + 22-stub skeleton exist; repo cut at first validated chapter |
...
|---|---|---|
| **Model lock** — lead (~27–35 B NVFP4 interactive) and stretch (~100 B) demo drivers, plus the embedding/rerank/Whisper set | landscape §6 names the lead/stretch shapes but not yet the three sourced candidates each — that expansion is still owed under A (#232); until it lands the dated numbers live in `nvidia-dgx-spark-research.md` §2, §4, §5 → Steven picks | open |
...
| 128 GB is shared by the LLM and the k3d cluster | A ~100 B model at ~94 % memory leaves nothing for NiFi/Kafka/Flink | `nvidia-dgx-spark-k3d-cso.md` §5 (the resident-set subtotal against 121 GB usable): the lead model + the full CSO stack must fit with headroom; the stretch model is a demo mode, not a resident | F, A |
