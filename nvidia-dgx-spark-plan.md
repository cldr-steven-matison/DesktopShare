...
| F | k3s + CSO operators (NiFi/Kafka/Flink) on aarch64; GPU-services cutover ladder | `nvidia-dgx-spark-k3s-cso.md` | [#238](https://github.com/cldr-steven-matison/DesktopShare/issues/238) | **executing 2026-08-27 (Phase 4)** — doc written 2026-08-26; the install is now real on the box: cert-manager, ingress-nginx with ssl-passthrough, CSM, CSA (SSB off), CFM, the box's own Kafka `my-cluster` with three topics, and `mynifi`. §4 carries the as-built block; the §4 ingress and NodePort open questions are closed |
...
|---|---|---|
| **Model lock** — lead (~27–35 B NVFP4 interactive) and stretch (~100 B) demo drivers, plus the embedding/rerank/Whisper set | **Locked 2026-08-27:** lead `nvidia/Qwen3.6-35B-A3B-NVFP4` on NVIDIA's vLLM playbook (`:8000`); stretch and the embedding/rerank/Whisper set still per landscape §6 / research §2 | **lead locked** |
