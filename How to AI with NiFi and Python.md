...
In this post, I am going to share the exact methodology I used to leverage AI for writing custom NiFi processors safely, ensuring my dataflows operate with my custom python.

### The Input: Example Fraud Python Script

This script logic assumes 2 cities and transactions over $10,000 are fraud.  This fraud model normally intends to be deployed on CML in workbench and invoked in NiFi via invokeHttp.  I tested all of this and it works just fine.   Unfortunately this is option is not available in kubernetes testing (focus of this post) outside of Cloudera Public Cloud.  This script serves as a bridge to ensure the same python responses can be tested and allow downstream test data to flow in non CML connected environments.
...
**The Pro Move:** Pin the AI in a strict, proven architectural skeleton for the NiFi wrapper.  `I am going to show you one right now!`  By "Pin" I mean I literally had to wrestle AI and pin it down with my first processor [example](https://cldr-steven-matison.github.io/blog/Custom-Processors-With-Cloudera-Streaming-Operators/).  I proved to AI it worked, then we created and confirmed this following base processor worked. Last but not least we moved forward with making the actual processor I needed.  💪

### Rule 2: Prove the Skeleton First

Before you introduce a single line of AI-generated business logic, deploy a bare-minimum structural template to the canvas. If the skeleton doesn't load and route data, your complex logic won't either.  This will also prove you know how to deliver and re-deliver a processor to the canvas in a series of versions for rapid testing in the NiFi UI.
...
Now fire up your cluster, open up a python script, and see if you can make it into a custom nifi processor!