...
**Screen2 targets the correct physical monitor — fixed 2026-07-19.** `browser_launcher.py` (`files/browser_launcher.py`) was launching Chrome with no explicit position, so it landed on the primary monitor by default — not the dedicated second screen. Fix: `[System.Windows.Forms.Screen]::AllScreens` confirmed the actual secondary monitor's bounds (`1280x720` at `(-1920,137)`), and the launch now passes `--window-position=-1920,137 --window-size=1280,720` before `--kiosk` (kiosk fullscreens whichever monitor the window is on at launch, so it has to be positioned there first). Verified via a direct Win32 `GetWindowRect` check on the actual launched window (not just assumed from the flags) — bounds matched exactly.
