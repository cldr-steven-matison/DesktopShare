...
- **Live-check before `!load`/`!matrix`:** `GET /helix/streams?user_login=<streamer>` inside `TwitchChatListenerProcessor` before emitting a flowfile — skip the kill/relaunch chain and reply "not live right now" if offline, or confirm+proceed if live. Not built.
