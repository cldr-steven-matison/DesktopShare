...
> **Status (2026-09-08):** **Ch22's source doc is now `nvidia-dgx-spark-cso-demos.md` — CSO Operators / DGX Demo Designs** ([#234](https://github.com/cldr-steven-matison/DesktopShare/issues/234)). Four demos, all built with EFM and NiFi on the box: the EFM inference front door, SparkLlmBridge, ReleaseVoteWatch + TelegramNotify, and flink-agents. The earlier draft described other devices' flows repointed at the box and is retired. Cloudera AI / NIM parity stays under Part VI (work-stream I), which Steven is re-scoping from the Mac; a Part for Cloudera AI on Data Services waits on a test environment.
>
> **Status (2026-09-03):** **Part V (local AI for development) now has real material behind it.** Work-stream L ([#294](https://github.com/cldr-steven-matison/DesktopShare/issues/294)) measured how much of Claude Code's work the box can take over, and moved the parts that hold up: looking up our own docs, digesting long logs, and pulling facts out of a doc. Proofreading stays with Claude. The write-up is `nvidia-dgx-spark-offload.md`, now the second source doc for Ch17. The local knowledge base itself (H, [#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240)) has been live on the box since 2026-08-27, so Ch15 and Ch16 are no longer waiting on bring-up. All three chapters are still stubs; the prose has not been written yet.
>
...
| **9** 🔲 | No | Cloudera Streaming Operators on aarch64 — install | `nvidia-dgx-spark-k3s-cso.md` · [#238](https://github.com/cldr-steven-matison/DesktopShare/issues/238) | Source doc `nvidia-dgx-spark-k3s-cso.md` written 2026-08-26 on NvidiaSpark-1 (#238, `f67cfeb`), in review. Chapter still a stub; field validation waits on on-box bring-up ([#235](https://github.com/cldr-steven-matison/DesktopShare/issues/235)). |
...
| **12** 🔲 | No | EFM agent class NvidiaSpark-1 | `nvidia-dgx-spark-efm-agent.md` · [#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239) | Source doc done — **#239 closed 2026-08-28.** §1–§2 fully field-validated on-box: class flow **consolidated to the single-handler router (flowVersion 5, #270 §2)** — one `HandleHttpRequest` on `:8190` fronts all four doors via a path→`target.url` map + one dynamic `InvokeHTTP` + one `HandleHttpResponse` (16 proc / 19 conn); all four doors + `:9936` meter return 200 over the LAN (`/transcribe` via multipart-reconstruction sub-branch), export `files/issue-226/flows/NvidiaSpark-1.designer-flow.json` (+ prose companion). Chapter prose still a stub (authored at guide time). |
| **13** 🔲 | No | Out-of-box edge-AI use cases — the Jetson → Spark ladder | `nvidia-dgx-spark-efm-agent.md` · [#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239) | Source doc done — **#239 closed 2026-08-28** (§3 = 10 use-case designs, Jetson→Spark escalation ladder). Chapter prose still a stub; end-to-end field-validation of a use case from a non-Spark device carried forward to on-box bring-up ([#235](https://github.com/cldr-steven-matison/DesktopShare/issues/235)). |
| **14** 🔲 | No | Observability: Prometheus exporters, the EFM fleet board, DGX Dashboard | `nvidia-dgx-spark-efm-agent.md` · [#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239) | Source doc done — **#239 closed 2026-08-28.** `:9936 /metrics` leg live on-box (`GET :9936/metrics` → 200, real `/proc` GB10 values). Cluster-side scrape (ServiceMonitor + fleet-board row) and `:9835` host exporter carried forward to observability wiring. Chapter prose still a stub. |
| **15** 🟡 | Partial | Local knowledge base for Claude Code (MCP + Qdrant) | `nvidia-dgx-spark-local-kb.md` · [#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240) | The knowledge base is built and running on `spark-dd06` (#240 closed 2026-08-27): our docs are indexed and Claude Code can query them through the `ds-kb` tool. Since 2026-09-02 a hook also hands a session the matching doc sections whenever it searches the repo (#294). Chapter prose still a stub. |
| **16** 🟡 | Partial | Local agentic validation loops | `nvidia-dgx-spark-local-kb.md` + `nvidia-dgx-spark-offload.md` · [#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240) / [#294](https://github.com/cldr-steven-matison/DesktopShare/issues/294) | The box checks a doc and flags problems; Claude decides which flags are real. Measured on real docs (#294, 2026-09-03): the box is a useful first pass on a doc with problems, but on a clean doc it invents findings, so Claude keeps the final say. Chapter prose still a stub. |
| **17** 🟡 | Partial | What moves off cloud tokens — cost control, measured | `nvidia-dgx-spark-local-kb.md` §5 + `nvidia-dgx-spark-offload.md` · [#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240) / [#294](https://github.com/cldr-steven-matison/DesktopShare/issues/294) | Measured, not guessed (#294, 2026-09-02/03). The box's share of Claude Code's work started at about 1.5 % and roughly doubled once log digestion and fact extraction moved over. What moves: doc lookups, long-log summaries, fact extraction. What stays with Claude: judgement, proofreading, writing. A scoreboard command records the share at the end of each session. Chapter prose still a stub. |
...
| **22** 🟡 | Partial | Demo catalogue | `nvidia-dgx-spark-cso-demos.md` · [#234](https://github.com/cldr-steven-matison/DesktopShare/issues/234) | Source doc rewritten 2026-09-08 as the four on-box EFM + NiFi flows, each live or field-validated with a committed export. Chapter stub not yet authored. |
...
- `nvidia-dgx-spark-offload.md` — Ch16, Ch17 (L, [#294](https://github.com/cldr-steven-matison/DesktopShare/issues/294)): what the box takes over from Claude Code, measured
...
- `nvidia-dgx-spark-cso-demos.md` — Ch22 (C, [#234](https://github.com/cldr-steven-matison/DesktopShare/issues/234)); the demos are the flows built with EFM + NiFi on the box, nothing from other devices
