...
| **14** 🟡 | Partial | Observability: Prometheus exporters, the EFM fleet board, DGX Dashboard | [`nvidia-dgx-spark-efm-agent.md`](nvidia-dgx-spark-efm-agent.md) · [#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239) | `:9936/metrics` leg live on-box. Cluster-side scrape is a WindowsDesktop apply: manifests (`nvidiaspark1-metrics.yaml`), fleet-board panels and the `:9835` `dgx-spark-prometheus` installer are in `files/issue-239/` (2026-09-10). Prose pending. |
