...
![GetIceberg reading poc_uc2.airlines live on a CDP Data Share — one FlowFile on the success relationship, running on the com.example nifi-iceberg-read-nar bundle](../images/GetIceberg-flow.png)
...
![QueryIceberg on poc_uc2.airlines — three SQL dynamic properties (all, by_dest, filtered) each routed to its own relationship of the same name](../images/QueryIceberg-flow.png)
...
![QueryFlights on the ~120k-row flights table partitioned by flight_month — the pruned / delayed / carrier_stats queries each on their own relationship, WHERE flight_month = '2026-03' reading one data file of twelve](../images/QueryFlights-flow.png)
