...
- **k3s platform / CSO operators** (`files/issue-226/spark-operators.sh` → `nvidia-dgx-spark-k3s-cso.md §4`): cert-manager 1.16.3, ingress-nginx 4.13.5 (host-network, `--enable-ssl-passthrough`, owns `:80`/`:443`), strimzi 1.6.0-b99 (mem raised to 1Gi — 384Mi default OOMKills here), csa-operator 1.5.0-b275 (`ssb.enabled=false`), cfm-operator 3.0.0-b126 — **the arm64 ceiling: `cfm-operator` ≥ 3.3.x is amd64-only** (single-manifest images; Cloudera eng confirmed 2026-09-15). A `helm upgrade` to `3.3.1-b15` crash-loops with `exec /manager: exec format error`; rolled back to `3.0.0-b126` (helm rev 3) with `mynifi` untouched (#180). Anything needing a newer operator (e.g. `spec.security.ranger`) runs on an x86 box. Namespaces `cld-streaming` / `cfm-streaming`; secrets `cloudera-creds` + `cfm-operator-license` (both), `nifi-admin-creds` (`cfm-streaming`).
