...
### Network
- **Current state (#334): the box is not on the array LAN.** Its Wi-Fi (`wlP9p1s0`) holds `192.168.1.144/24` on a *different* 192.168.1.x network (a `2605:59c0:…` IPv6 prefix vs WindowsDesktop's `2600:1702:…`), so `192.168.1.203` answers "No route to host" from WindowsDesktop and `192.168.1.121:10090` is unreachable from the box. **Every path to the box is the tailnet `100.104.155.57` until it is back on the array LAN** — SSH, the `:8190` doors, `:8000`. The agent's C2 base is on the tailnet (above). The `192.168.1.203` facts below are the pre-move state.
