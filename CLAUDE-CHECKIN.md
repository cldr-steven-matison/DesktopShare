...
- **Reboot autostart (2026-09-16):** logon task `WindowsDesktop-Kube-AutoStart` (`files/windowsdesktop-kube-autostart.xml`, fires 1 min after sign-in; re-register recipe in its `Description`) opens a Windows Terminal window running `files/windowsdesktop-kube-boot.sh`: wait for Docker Desktop (autostart is on — `Run` key + `AutoStart: true`), `minikube start` on the active profile unless it is already Running, 5 min settle, then `zellij --layout kube-service-ports-efm --session kube-service-ports-efm` — or attach if the layout is already up, never a second set of forwards. Log: `~/.cache/kube-boot/boot.log`; manual test `DS_KUBE_BOOT_SETTLE=20 bash files/windowsdesktop-kube-boot.sh`. **Needs a Windows sign-in**: `AutoAdminLogon` is `0`, so after a reboot nothing runs until Steven signs in (autologon via `netplwiz` would close that gap).
- **Zellij layout `kube-service-ports-efm.kdl`:** the first pane is `minikube tunnel`, which runs one `sudo ssh -N … -L 80 -L 443` to bind the privileged ports. `/etc/sudoers.d/minikube-tunnel` (`files/windowsdesktop-minikube-tunnel.sudoers`, install command in its header) lets exactly that command pass without a password; if the pane ever sits on a sudo prompt again, that file is missing (`sudo -n -l | grep ssh`) — unanswered it takes EFM and every forward down and looks like a dead cluster, so check it first (`agent/incident-rules.md` "Port-forwards"). All forward panes self-heal in a `while true` loop.
...
### Key tool versions
- Git 2.43.0 · Python 3.12.3 · Docker 29.2.1 · CUDA 13.0 (`nvcc` V13.0.88, `/usr/local/cuda`→`cuda-13.0`) · jq 1.7 · node · OpenJDK 21.0.12 · `gh` 2.98.0 · `kubectl` v1.32.13 (CSA/CSM 1.32 ceiling) · `helm` v3.21.4 — last three user-local in `~/.local/bin` · Maven 3.9.10 user-space at `~/.local/opt/apache-maven-3.9.10` (`~/.local/bin/mvn`, no sudo); `~/.m2` seeds the CFM `nifi-iceberg-services-api:2.6.0.4.3.4.0-234` jar + NAR the `nifi-iceberg-read-bundle` build pins (bootstrapped off the pod, #355).
...
- **NiFi** `mynifi` in `cfm-streaming` — NiFi 2.6.0 / CFM 3.0.0-b126, `files/issue-226/nifi-spark.yaml` (`local-path` repos, `spec.resources.nifi` 8Gi, userCertAuth + S2S). Admin identity `nifi-admin` (client cert, SAN `nifi-admin`). → `nvidia-dgx-spark-k3s-cso.md §6`. **Custom NAR:** `nifi-iceberg-read-nar-1.0.3-SNAPSHOT.nar` hot-loaded in `/opt/nifi/nifi-current/data/extensions` (`GetIceberg` + `QueryIceberg`; NiFi won't re-register a same-version NAR — bump the bundle version before any redeploy). **`IcebergRESTCatalogDemo` PG** on the canvas (x=1424, Iceberg row) with Parameter Context `iceberg-demo-params` (`client_id`/`client_secret` from `credentials.json` in the `iceberg-rest-catalog-demo` clone — that file, not `credentials-nifi.json`, is the working set; the clientId churns on every rebuild, re-push with `rewire-nifi-creds.sh`). Export `files/issue-355/flows/IcebergRESTCatalogDemo.flow.json`. Delete via the API needs the revision as a **query param** (`DELETE …?version=N`), not a JSON body.
...
- **k3s `node-ip` is pinned to one SSID's address** in `/etc/rancher/k3s/config.yaml` (`node-ip:`; its `tls-san` lists both `.144` and `.203`, so the API cert covers either). On the other SSID k3s exits on every start — `journalctl -u k3s` → `failed to find interface with specified node ip`, `NRestarts` climbing, `127.0.0.1:6443` refused — and every API-dependent pod flaps with it. Check: `grep node-ip /etc/rancher/k3s/config.yaml` against `ip -4 -br addr show wlP9p1s0`; fix: re-pin to the live address, `sudo systemctl restart k3s` (#352).
- **Corp VPN and k3s:** `gpclient` (GlobalProtect, `tun0`) installs `default dev tun0 scope link` with no route for the service CIDR, which would send `10.43.0.0/16` into the tunnel — and it swallows pod Internet egress too, where the corp firewall TLS-resets Twitch and Kick (StreamerResearch's Helix processors go red at `OAuth2 access token request failed … Connection reset`). `k3s-vpn-route.timer` (30 s; units + `install.sh` in `files/issue-352/`) keeps `10.43.0.0/16 via <LAN gw> dev wlP9p1s0` **and** policy-routes pod sources (`from 10.42.0.0/16` → rules 5300/5301 → table 100, default via the LAN gateway, `10.80.0.0/16` goes01 kept on `tun0` for #351). Checks: `ip route get 10.43.0.1` and `ip route get 52.85.193.18 from 10.42.0.10 iif cni0` must not say `dev tun0`; `ip route get 10.80.1.1 from 10.42.0.10 iif cni0` must (VPN up); `systemctl list-timers k3s-vpn-route.timer` (#352, addendum in its as-built).
- **A pod's public egress is not the host's** (consequence of the policy routing above, #355): with the VPN up the host's `curl` leaves on the corp pool (`165.1.200.x`) but a k3s pod leaves on the home ISP address. Any cloud allow-list that NiFi/Kafka/Flink must pass through (the `srm-iceberg` Knox SG on 443, a CE ingress prefix list) needs the **pod's** IP — a host-side `/32` alone leaves the pod timing out while the host's curl succeeds. Check both: `curl -s https://api.ipify.org` vs `kubectl -n cfm-streaming exec mynifi-0 -c nifi -- curl -s https://api.ipify.org`.
