...
- **NiFi UI from a browser (2026-08-27)**: not yet solved — the API answers 200 over the LAN with the admin client cert and no tunnel, but a browser needs a hosts entry (Ingress routes by SNI on `mynifi-web.mynifi.cfm-streaming.svc.cluster.local`) and the `nifi-admin` client cert. **Any different hostname gets `400 Invalid SNI`** — `nifi.web.proxy.host` is a fixed whitelist on the CR. Options and the recommendation are in [#257](https://github.com/cldr-steven-matison/DesktopShare/issues/257). **This device has no zellij and no port-forward layout**, and does not need one for the UI: k3s binds host ports directly.
