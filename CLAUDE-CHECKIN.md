...
- **Reboot survival (#322, 2026-09-10):** every service on the box has a boot owner; `files/issue-322/` holds it, `sudo files/issue-322/install.sh` deploys it (idempotent; `--cold-start` also destroys the six containers and starts the boot unit, no reboot).
  ...
  Logs: `journalctl -u nvidia-serve-boot` · `journalctl -u minifi-java` · `journalctl -u nvidia-post-boot-verify`. Deployed state on the box is recorded on #322.
  **NVMe APST-off flag (2026-09-12, #322):** the box froze 1–2.5 min after every boot (NVMe controller stopped answering, `Device not ready; aborting reset`); `/etc/default/grub.d/nvme-apst-off.cfg` now puts `nvme_core.default_ps_max_latency_us=0 pcie_aspm=off` on the kernel line — do not remove it. **Off-switch for the Docker tier, no sudo:** `touch ~/.nvidia-serve-boot.off` keeps `nvidia-serve-boot` from starting the six containers; `rm` it to re-arm. **Triage from another device** in the short window after a boot: `ssh tunas@192.168.1.203 bash -s < files/issue-322/lockup-triage.sh` (or `storage-triage.sh`, `nvme-facts.sh`); `kernel.dmesg_restrict=1` here, so `dmesg` needs root — `journalctl -k` works as `tunas`. Runbook §8.
