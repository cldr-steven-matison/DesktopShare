...
- **Reaching DGX Spark NiFi (spark-dd06) from this device — tailnet-only, no tunnel (#257, 2026-08-28).** The box is **not** on StarlinkAI's LAN (StarlinkAI is `192.168.1.245/24` on Starlink; the box is `192.168.1.203/24` on a different physical `/24`), so the only path is the **tailnet IP `100.104.155.57`** — there is no port-forward, tunnel, or NodePort hop from here to reach it; the box's ingress-nginx runs host-network with `--enable-ssl-passthrough`, so a TLS session against `100.104.155.57:443` terminates on NiFi's own Jetty and the client cert travels end-to-end. **SNI must be `mynifi-web.mynifi.cfm-streaming.svc.cluster.local`** — any other name → `400 Invalid SNI` at Jetty (or `404` nginx default backend). Client identity lives in **`~/nifi-admin.p12` + `~/ca.crt`** (delivered by WindowsDesktop 2026-08-27; **never committed — whoever holds the p12 is `nifi-admin`**), p12 password `nifi-admin`, admin cert expires **2026-11-25** (cert-manager renews on the box; re-copy the two files after each renewal). Verified end-to-end from WSL 2026-08-28: `--resolve …:443:100.104.155.57` with the client cert → `/nifi-api/flow/current-user` 200 `identity=nifi-admin`, `/nifi/` 200 text/html. **From WSL use `curl --resolve`, not an `/etc/hosts` entry** — WSL's `/etc/hosts` is auto-generated (`generateHosts`) and there is no browser or NSS store in this WSL environment; the human browser on this device is on the **Windows host** (hosts entry + Firefox/Chrome cert import against the same tailnet IP + SNI name — Steven's manual step).
