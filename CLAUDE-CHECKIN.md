...
- **Reaching DGX Spark NiFi (spark-dd06) from this device — tailnet-only, no tunnel (#257, 2026-08-28).** The box is **not** on StarlinkAI's LAN (StarlinkAI `192.168.1.245/24` on Starlink; box `192.168.1.203/24`), so the only path is the **tailnet IP `100.104.155.57:443`** — no port-forward/tunnel/NodePort hop. The mTLS mechanism itself (host-network ingress-nginx + `--enable-ssl-passthrough`, SNI **must** be `mynifi-web.mynifi.cfm-streaming.svc.cluster.local` or `400 Invalid SNI`, the client cert as identity, and the renewal cadence) is documented once in the **NvidiaSpark-1 block below** (§"NiFi UI from a browser") — read it there rather than maintain a second copy. StarlinkAI-specific: client identity in **`~/nifi-admin.p12` + `~/ca.crt`** (delivered by WindowsDesktop 2026-08-27; never committed — whoever holds the p12 is `nifi-admin`), p12 password `nifi-admin`. **From WSL use `curl --resolve …:443:100.104.155.57`, not `/etc/hosts`** — WSL's `/etc/hosts` is auto-generated and there's no browser/NSS store here; the human browser is on the **Windows host** (hosts entry + cert import against the same tailnet IP + SNI name). Verified from WSL 2026-08-28: `/nifi-api/flow/current-user` 200 `identity=nifi-admin`.

### Repo homes on this host
- DesktopShare: `~/Brainshare` — **the clone was renamed `DesktopShare` → `Brainshare` locally** (2026-09-02, #288). The memory silo keys off this path (live silo `~/.claude/projects/-home-tunas-Brainshare/memory`); the stranded `-home-tunas-DesktopShare` silo was migrated in during the #288 sweep.
...
**Prod profile `cso-prod-1` (cutover EXECUTED 2026-08-26, #253):** pre-prod validation passed 2026-08-25 (`files/cso-prod-1/VALIDATION.md` — cert-manager v1.16.3, CFM 3.0.0-b126 / NiFi 2.6.0, Strimzi CSM 1.6.0-b99, upstream flink-kubernetes-operator 1.13.0, vLLM `Qwen/Qwen2.5-7B-Instruct-AWQ`, Flink Agents `cso-operator-flink-agents:0.3.1`); the cutover in `cso-prod-1-cutover-plan.md` then ran 2026-08-26, so **the services listed below are now `cso-prod-1`'s** (see the role bullet at the top of this block — the old default `minikube` profile is the stopped-on-disk rollback). The 13 prod root PGs are exported under `files/cso-prod-1/flows/prod/`.
...
- **Postgres `streamers` roster for the DGX Spark caption brain** (#276, 2026-08-30): `cld-streaming` ns, `svc/ssb-postgresql` `5432`, paired panes `ssb-postgresql-121:5432` (LAN `192.168.1.121`) / `ssb-postgresql-126:5432` (tailnet `100.68.113.126`), added to the running session + `.kdl` with `files/agent-kube-forward.sh` (no restart). Role `streamer_brain` can SELECT the `streamer_brain` view only; its password lives in `~/.env` on spark-dd06. **Verified from spark-dd06 (both paths open, asyncpg 17 rows via the view):** the tailnet path needed no firewall change; the LAN path needed the elevated `netsh advfirewall firewall add rule name="Postgres streamers 5432" dir=in action=allow protocol=TCP localport=5432` on the Windows side (same gap as Mosquitto/1883 below) — Steven added it 2026-08-30.
...
- **Checked in**: 2026-08-26 — box landed and booted; block filled from the real host (`nvidia-smi`, `uname -a`, `free -g`, `df -h`, `lscpu`). Hostname arm `spark-dd06*` → `NvidiaSpark-1` added to `ds_device_labels()` in `.claude/hooks/lib-device.sh` the same session, so the SessionStart inbox check maps this host from next session on. **Phase-3 arrival-day work completed 2026-08-27/28** (per `nvidia-dgx-spark-plan.md`): k3s + `kubectl`/`helm` installed, Tailscale joined, EFM agent enrolled, and the serving endpoints stood up — all documented in the sections below. **Still open:** the static IP reservation (see Network, below). **`gh` is installed** (2026-08-26, v2.98.0 arm64 at `~/.local/bin/gh`, authed as `TunaStreetTest`) so the SessionStart inbox check works from here — but `~/.local/bin` is not on the hook's non-login PATH, so `lib-device.sh` now prepends it (and `/usr/local/bin`, `/opt/homebrew/bin`) at source time for both checkin.sh and guard.sh.
...
- **Installed 2026-08-27 (system, `apt`)**: `libnss3-tools` — `certutil`/`pk12util`, so an NSS-store browser profile (Firefox anywhere, Chrome on Linux) can be loaded with the `nifi-admin` client cert non-interactively instead of clicking through Settings; the GUI import is what was actually used the first time.
- **Installed 2026-08-27 by `files/issue-226/spark-bootstrap.sh`**: k3s `v1.32.13+k3s1` (systemd `k3s`, own containerd 2.1.5, `nvidia` RuntimeClass auto-created, traefik disabled, kubeconfig `/etc/rancher/k3s/k3s.yaml` mode 644), OpenJDK 21.0.12 (for MiNiFi Java), Tailscale (installed; join pending the browser login), ufw (deny-in; 22/8000 from `192.168.1.0/24`, tailnet, k3s CIDRs — the NodePort list was corrected 2026-08-27 to this box's own block 32100·32101·32102·32103 plus 80/443 for ingress-nginx; the first run carried prod's 31623·31850·31935·30336 by mistake and needs a re-run to apply), Docker `nvidia` runtime registered (default stays `runc`), `tunas` in `docker` group. `minikube` is not planned here.
...
- **Serving tier — embed / rerank / STT (2026-08-28, [#232](https://github.com/cldr-steven-matison/DesktopShare/issues/232))**: the RAG + captioning parity set, co-hosted with the lead on the LAN address + loopback (Docker-bound, ufw-consistent). **`:8001`** `BAAI/bge-m3` 1024-d embeddings via TEI (container `tei-embed-bge`, `files/issue-226/tei-embed-serve.sh`) — `/embed`, ~7 GB; **`:8002`** `BAAI/bge-reranker-v2-m3` rerank via TEI `/rerank` (container `tei-rerank-bge`, `tei-rerank-serve.sh`) — ~5 GB; **`:8003`** whisper.cpp `large-v3` CUDA STT (container `whisper-cpp`, `whisper-serve.sh` + `files/issue-226/whisper/`, built with `CMAKE_CUDA_ARCHITECTURES="120;121"`) — serves **`/inference`** (multipart `file=@…`); **`/v1/audio/transcriptions` 404s on this build** (corrected 2026-08-28 — the earlier "OpenAI + /inference" note was wrong). All three use the same sm_121-native TEI image proven by `tei-kb`. The **stretch** `nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4` is a **swap-in on `:8000`** (stops the lead first), `files/issue-226/vllm-stretch-serve.sh` — not resident. Full model lock + candidates: `nvidia-dgx-spark-landscape.md` §6.
- **EFM agent (2026-08-27)** — the agent row: version **MiNiFi Java `2.24.08.0-19`**, class **`NvidiaSpark-1`**, `agentIdentifier` **`c4870255-7136-4eef-837a-70f127733b38`** (server-minted by `generateCommand`), install dir **`/home/tunas/minifi-2.24.08.0-19`**, service **`minifi-java`** (SysV via systemd generator, root → `tunas`), log **`/home/tunas/minifi-2.24.08.0-19/logs/minifi-app.log`**, listener ports **`:8190`** (all four inference routes) + **`:9936`** (`/metrics`). Heartbeats to `http://192.168.1.121:10090/efm/api` over the LAN; manifest `d81ca4b5-…` attached to the class. Enrollment command kept at `~/minifi-java-deploy/enroll-NvidiaSpark-1.sh` (do not re-run — the identifier is spent). **Class flow built, published & consolidated 2026-08-28 ([#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239), [#270](https://github.com/cldr-steven-matison/DesktopShare/issues/270))**: AI model router. First built as four separate `HandleHttpRequest→InvokeHTTP→HandleHttpResponse` legs on `:8190–:8193` (flowVersion 3–4), then **consolidated to a single-handler router at flowVersion 5** — **ONE `HandleHttpRequest` on `:8190` fronts all four routes** (`/reason`→:8000, `/embed`→:8001, `/rerank`→:8002, `/transcribe`→:8003/inference) via a path→`target.url` map into one dynamic `InvokeHTTP` and one `HandleHttpResponse`; `/transcribe` keeps its multipart-reconstruction sub-branch. **`:8191/:8192/:8193` no longer listen — all doors answer on `:8190/<path>`.** `:9936 /metrics` unchanged. All bound `0.0.0.0`, LAN-reachable; all four doors + metrics return 200 (send `/reason` the real vLLM model id — an unknown id 404s at the upstream, not a flow fault). 16 proc / 19 conn. Export `files/issue-226/flows/NvidiaSpark-1.designer-flow.json` (+ `.flow-notes.md` prose companion).
...
- **k3s platform (2026-08-27, Phase 4 / #238)**: cert-manager `v1.16.3`, ingress-nginx `4.13.5` (**host-network, `--enable-ssl-passthrough`** — controller owns the box's `:80`/`:443`; `:442` is its internal TLS leg), the three CA objects from `files/cso-prod-1/cluster-issuer.yaml`, strimzi-cluster-operator `1.6.0-b99` (**memory raised to 1Gi — the chart's 384Mi default OOMKills it here**), csa-operator `1.5.0-b275` with `ssb.enabled=false` (only `flink-kubernetes-operator` resident), cfm-operator `3.0.0-b126`. Namespaces `cld-streaming` / `cfm-streaming`, secrets `cloudera-creds` + `cfm-operator-license` in both, `nifi-admin-creds` in `cfm-streaming`. Whole install is `files/issue-226/spark-operators.sh` (idempotent, step-by-name), reading `~/.cloudera-creds` (mode 600) and `~/license.txt`.
- **Kafka on this box (2026-08-27)**: its own `my-cluster` in `cld-streaming` — 3 combined KRaft nodes, `local-path` 20Gi each, `files/issue-226/kafka-spark.yaml`. **Own NodePort block, not prod's**: bootstrap `192.168.1.203:32100`, brokers `32101`/`32102`/`32103`, `advertisedHost` = `192.168.1.203` (k3s NodePorts are host ports — no tunnel). Topics `spark-inference-requests`, `spark-inference-results`, `spark-kb-documents` (`files/issue-226/kafkatopics-spark.yaml`).
- **NiFi on this box (2026-08-27)**: `mynifi` in `cfm-streaming`, CFM operator 3.0.0-b126 / NiFi 2.6.0, `files/issue-226/nifi-spark.yaml` — `local-path` repos (content 50Gi, provenance 20Gi, flowfile/data 10Gi, state 5Gi), `spec.resources.nifi` 8Gi, userCertAuth + S2S day one off `cfm-operator-ca-issuer-signed`. Admin identity `nifi-admin` from `files/cso-prod-1/user-nifi-admin.yaml` (client cert, SAN `nifi-admin`).
- **SparkLlmBridge flow (2026-08-27, the Phase-4 gate)**: the first PG on this box's `mynifi` — `ConsumeKafka(spark-inference-requests)` → `EvaluateJsonPath` → `ReplaceText` → `InvokeHTTP POST #{vLLM Base URL}/v1/chat/completions` → `PublishKafka(spark-inference-results, key ${request_id})`, with every failure path to one `LogAttribute` and `Retry` self-looped at a 10-minute expiry. Parameter Context `SparkLlmBridge` holds `vLLM Base URL` = `http://192.168.1.203:8000`, `Kafka Bootstrap` = `my-cluster-kafka-bootstrap.cld-streaming.svc:9092` (the **internal** listener — `32100` is for off-box clients), `LLM Model`. Export: `files/issue-226/flows/SparkLlmBridge.json`. Gotcha: `max_tokens` must be sized for a reasoning model — at 256 the reply came back `"content": null` with the budget spent in `reasoning`; 2048 works.
- **Flink on the GPU (2026-08-27)**: `files/issue-226/flink-gpu.yaml` — stock CSA Flink `1.20.1-csaop1.5.0-b275` (multi-arch, no custom image), GPU limit in the **taskManager `podTemplate`** on container `flink-main-container` plus `runtimeClassName: nvidia`. Proven: `nvidia-smi` inside the TM returns `NVIDIA GB10` / driver `580.173.02`, and Flink logs `Enabled external resources: [gpu]` → `GPUDriver: Discover GPU resources: [GPU Device(0)]`. **A session cluster with no job creates no TaskManager**, so submit a job to see the claim. Not left running — it would hold the box's only GPU allocation. Destroy path: cancel jobs first, then delete (2.4 s clean); with jobs live the finalizer hangs on `CLEANUPFAILED`.
- **flink-agents 0.3.1 (2026-08-27)**: image `spark-flink-agents:0.3.1` (4.35 GB, arm64) built from `files/issue-226/flink-agents/Dockerfile` — prod's recipe with **no aarch64 changes**. k3s uses its own containerd, so it needs `sudo bash -c 'docker save spark-flink-agents:0.3.1 | k3s ctr images import -'` before a `pullPolicy: Never` deploy. `FlinkDeployment/flink-agents` `STABLE`; the example job runs clean against `http://192.168.1.203:8000/v1/chat/completions` — 133+ calls, zero exceptions, where the same job FAILED on cso-prod-1's 3B/7B. `OPENAI_COMPLETIONS_*` in flink-agents is the **chat** completions API (the box's raw `/v1/completions` emits `<think>` inline and would break `json.loads`).
- **Flink test deployments are NOT resident (2026-08-27)**: both `flink-gpu` and `flink-agents` were torn down after proving their gates — `flink-gpu` holds the box's only `nvidia.com/gpu` allocation, and neither needs to run idle. Re-deploy either with one `kubectl apply -f files/issue-226/flink-gpu.yaml` / `files/issue-226/flink-agents/flinkdeployment.yaml`. The `spark-flink-agents:0.3.1` image stays in both the Docker daemon and k3s's containerd, so no rebuild is needed. **Destroy order matters**: cancel jobs first, then delete the FlinkDeployment (2.4 s and 7.0 s clean); with jobs live the finalizer hangs on `CLEANUPFAILED`.
- **NiFi UI from a browser (2026-08-27, [#257](https://github.com/cldr-steven-matison/DesktopShare/issues/257) option A — works)**: `https://mynifi-web.mynifi.cfm-streaming.svc.cluster.local/nifi/` — **no tunnel, no port-forward, nothing deployed**. ingress-nginx is host-network with `--enable-ssl-passthrough`, so the browser's TLS session terminates on NiFi's own Jetty via the box's `:443` and the client cert travels end to end. Two one-time client-side steps, both done off `files/issue-226/nifi-admin-p12.sh` (exports secret `nifi-admin-cert` to `~/nifi-admin-spark/` and re-verifies the live path — it deploys nothing and touches no running service): **(1)** a hosts entry `192.168.1.203  mynifi-web.mynifi.cfm-streaming.svc.cluster.local` on whichever machine runs the browser — the Ingress routes by SNI on that name, and `nifi.web.proxy.host` answers **`400 Invalid SNI`** to every other name, so no shortcut hostname works; on the box itself `127.0.0.1` resolves just as well (verified 200). **(2)** import `ca.crt` as a trusted authority and `nifi-admin.p12` as the client certificate (password `nifi-admin`; the file's mode 600 is the real control — whoever holds it **is** `nifi-admin`). Verified 2026-08-27 on the box: `/nifi-api/flow/current-user` → `200 identity=nifi-admin` and `/nifi/` → `200 text/html` through `:443` with both the PEM pair and the p12. **Confirmed end to end in a browser 2026-08-27**: Steven added the hosts entry on the box, imported `ca.crt` (Authorities) and `nifi-admin.p12` (Your Certificates) into Firefox 154's own store (profile `~/snap/firefox/common/.mozilla/firefox/cuq12k8z.default` — Firefox does not use the OS trust store), and the canvas loads as `nifi-admin`. With the hosts entry in place `curl` needs no `--resolve` at all: `api 200 via 192.168.1.203:443`. **The admin cert is 90-day** — cert-manager renews it 2026-10-26; re-run the script after each renewal or the imported copy starts failing. Option B (`files/cso-prod-1/nifi-ui-proxy.yaml`) was deliberately **not** ported: prod keeps that proxy on loopback because whoever reaches it is `nifi-admin` with no auth in front, and on this box it would have to sit on a LAN-reachable Ingress — a security decision to take deliberately, not a convenience.
- **Ports and tunnels on this device (2026-08-27, #257)**: **k3s binds real host ports, so this box has no tunnel layer at all** — there is no `minikube tunnel` in any path and nothing needs forwarding to be reachable from the LAN. The published surface is `:80`/`:443` (ingress-nginx, host-network), `:8000` (vLLM, docker-published on loopback + LAN), `32100`–`32103` (Kafka NodePorts) and `:6443` (k3s API). **So this device gets no zellij and no `kube-service-ports-*.kdl`, and no session should ever start a background `kubectl port-forward` here** — `agent/incident-rules.md`'s port-forward rule applies as its plain "don't start one": if something genuinely is not reachable, the fix is an Ingress or a NodePort in a committed yaml, not a background process a session owns. (`kubectl port-forward` in the foreground for a one-off debug is fine — it dies with the command.) The ufw allow-list for these ports is as `spark-bootstrap.sh` last wrote it (see the bootstrap line above — the NodePort correction needs its re-run); it was not re-read this session, `/etc/ufw/user.rules` being root-only, so if a LAN browser times out on `:443`, check that first. Off-LAN access stays Tailscale's job and is deliberately unconfigured: `tailscale serve` would present `nvidiaspark-1.tail1f447b.ts.net` to NiFi and earn `400 Invalid SNI`, so it needs a `nifi.web.proxy.host` CR edit and a `mynifi` restart first (#257 option C — not done, and a confirm-first change).

- **Local KB (2026-08-27, [#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240) work-stream H, retrieval half)**: the `ds-kb` knowledge base is BUILT and localhost-only on this box. **Qdrant** container `qdrant-kb` on `:6333` (`/home/tunas/kb/qdrant`), **TEI** container `tei-kb` on `:8080` serving `nomic-ai/nomic-embed-text-v1` at 768-d — the **prebuilt `ghcr.io/huggingface/text-embeddings-inference:121-latest` (sm_121) tag runs native, no `CUDA_COMPUTE_CAP=121` build needed** (settles the local-kb doc's open question). Collection **`desktopshare-kb`** (768-d Cosine, unnamed vector — matches the fleet's `my-rag-collection` shape), **4197 chunks** over the §2 corpus (plan/completed/blog/rule/chapter/flow/code). Docs embedded with the nomic `search_document: ` prefix, queries with `search_query: `. Query it with the **`kb_search`** MCP tool (args `query`, `kind=`, `repo=`, `limit=`). Server `/home/tunas/kb/kb_mcp.py` (FastMCP over stdio, run via `uv run --with 'mcp<2'` — the `mcp` package is 2.x now and renamed FastMCP, so it is pinned `<2`); registered project-scope in `.mcp.json` as `ds-kb` (**pending the one-time interactive approval in `claude`**). Rebuild: `python3 files/issue-226/kb/ingest.py` (dependency-free, stdlib only); incremental `files/issue-226/kb/reindex.sh` runs from `.claude/hooks/checkin.sh` step 1c on changed `*.md`/`*.flow.json` — **spark-dd06-guarded and backgrounded, a hard no-op on every other device**. Not indexed: `my-rag-collection` (the demo app's live collection — never write to it).
- **Local validator (2026-08-27, #240 §4 / H4–H5)**: `files/issue-226/kb/validator.py` — the box's own model reviews a proposed command against our cardinal rules (grounded by a live KB retrieval) and returns a `{verdict,rule,reason}` JSON. **Runs with reasoning OFF** (`chat_template_kwargs:{"enable_thinking":false}`) — on, a verdict costs ~25 s and returns `content:null`; off, ~0.7–0.9 s. `--selftest` passes the ten-command gate **6/6 violations, 4/4 clean, <1 s**. **Wired into `guard.sh` as rule 10.5** — advisory only (`emit_ctx`, an `allow`+context, never a deny/ask), risky-shape-gated, **spark-dd06-only**, endpoint-checked, hard `timeout`, fail-open, `DS_VALIDATOR=0` to disable. Verified firing in-session on a `kubectl delete pod mynifi-0` payload; `git status` falls through. **Blocking stays off** until a clean one-week soak (§6 H5) — and a hook that times out is a pass, so it stays advisory by design.
- **Token measurement (2026-08-27, #240 §5)**: `files/issue-226/kb/measure.py` runs a movable workload (`lint`/`extract`) on the box's model and prices the same tokens at published rates, with `doc-check.py` as the quality anchor. Seed result (lint on `nvidia-dgx-spark-efm-agent.md`): local **6.8 s / 13.8k tok / $0** vs a hosted Sonnet agent **~77 s / 53.8k tok**; local ~500× cheaper and ~10× faster, hosted clearly higher-quality (5 real style findings vs a local false positive) — the "move the mechanical half, keep judgment hosted" verdict, measured. Full authoring-chain baseline still owed.
...
### Repo homes on this host
- DesktopShare: `/home/tunas/BrainShare` — **the clone was renamed `DesktopShare` → `BrainShare` locally** (2026-09-02, #288). Claude Code keys the memory silo off this path, so the live silo is `~/.claude/projects/-home-tunas-BrainShare/memory`; the old `-home-tunas-DesktopShare` silo was stranded and migrated in during the #288 sweep. Every other repo is still directly under `/home/tunas/`.
...
- **Streamers demo services on this box (2026-08-31, [#272](https://github.com/cldr-steven-matison/DesktopShare/issues/272)/[#271](https://github.com/cldr-steven-matison/DesktopShare/issues/271)/[#282](https://github.com/cldr-steven-matison/DesktopShare/issues/282) — Streamers track, NOT the DGX guide's serving tier)**: Qdrant collection **`streamer-kb`** on `qdrant-kb :6333` (bge-m3 1024-d, 46 points over 19 streamers, pronoun-free by construction — seeded by `files/streamers/kb/seed_profiles.py`); **`clip-prep`** k3s pod in ns `streamers` (`files/streamers/clip-prep.yaml`, image `clip-prep:0.1` built with docker + `k3s ctr images import`, NodePort **`:32110`**, `POST /prep` MP4 → transcript via whisper `:8003` + frame URLs it hosts); **`StreamerBrain` PG** on `mynifi` (export `files/streamers/flows/StreamerBrain.json`, generator `build_streamer_brain.py`; Parameter Context `StreamerBrain` holds the `streamer_brain` Postgres password, set via API from `~/.env`) fronted by NodePort **`:32111`** (`files/streamers/streamer-brain-door.yaml`) — `POST /caption`, headers `X-Clip-Id`/`X-Streamer`/`X-Source`/`X-Title`, body raw MP4, ~6 s per clip; reachable on LAN and tailnet. WindowsDesktop's shadow mode (#277) targets this door.
