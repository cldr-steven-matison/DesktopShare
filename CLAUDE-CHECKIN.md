...
- **EFM agent (2026-08-27)**: MiNiFi Java `2.24.08.0-19` enrolled as class `NvidiaSpark-1` via `generateCommand` (server-minted identifier); installed at `/home/tunas/minifi-2.24.08.0-19`, service `minifi-java` (SysV via systemd generator, root → `tunas`), heartbeating to `http://192.168.1.121:10090/efm/api` over the LAN; manifest `d81ca4b5-…` attached to the class. Enrollment command kept at `~/minifi-java-deploy/enroll-NvidiaSpark-1.sh` (do not re-run — the identifier is spent). **Class flow v1 built + published 2026-08-28 ([#239](https://github.com/cldr-steven-matison/DesktopShare/issues/239))**: AI model router (StarlinkAI-style `HandleHttpRequest→InvokeHTTP→HandleHttpResponse`), flowVersion 3, listeners **`:8190 /reason`→:8000, `:8191 /embed`→:8001, `:8192 /rerank`→:8002, `:8193 /transcribe`→:8003/inference, `:9936 /metrics`** (all bound `0.0.0.0`, LAN-reachable). **flowVersion 4: all four inference doors + `/metrics` return 200** — `/transcribe` runs a 10-processor multipart-reconstruction leg (cloned from StarlinkAI). Export `files/issue-226/flows/NvidiaSpark-1.designer-flow.json`.
