...
- **TelegramNotify PG** (#289, → `nifi-release-vote-automation.md`; **artifacts built, live upsert + extension rollout pending confirm**): reusable root-level notifier for `mynifi` — `notify-in` input port → custom Python processor `SendTelegram` (`files/issue-289/processors/SendTelegram.py`, mounted via the `custom-python-extensions` PVC → `nifi.python.extensions.directories`). `ReleaseVoteWatch` feeds it through its `telegram-out` output port. Parameter Context `TelegramNotify` holds the shared bot creds; `~/.env` gains keys **`TOKEN`** + **`CHAT_ID`** (names only, never values — pending WindowsDesktop delivery #293). Runs in Dry Run until the creds land.
