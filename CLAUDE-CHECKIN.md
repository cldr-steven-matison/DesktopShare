...
- **Corp VPN and k3s:** `gpclient` (GlobalProtect, `tun0`) installs `default dev tun0 scope link` with no route for the service CIDR, which would send `10.43.0.0/16` into the tunnel. `k3s-vpn-route.timer` (30 s; units + `install.sh` in `files/issue-352/`) keeps `10.43.0.0/16 via <LAN gw> dev wlP9p1s0`. Check: `ip route get 10.43.0.1` must not say `dev tun0`; `systemctl list-timers k3s-vpn-route.timer` (#352).
