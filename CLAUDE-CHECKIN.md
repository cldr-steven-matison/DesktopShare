...
- **k3s bring-up** (`files/issue-226/spark-bootstrap.sh` → `nvidia-dgx-spark-k3s-cso.md §3`): k3s `v1.32.13+k3s1` (systemd, own containerd 2.1.5, `nvidia` RuntimeClass auto-created, traefik disabled, kubeconfig `/etc/rancher/k3s/k3s.yaml` mode 644); NVIDIA device plugin 0.20.0, node capacity `nvidia.com/gpu: 1`. ufw deny-in (22/8000 from `192.168.1.0/24` + tailnet + k3s CIDRs). **Open item:** the ufw NodePort block was corrected to this box's own `32100·32101·32102·32103` + 80/443, but the first bootstrap run wrote prod's `31623·31850·31935·30336` — a re-run is owed to apply it. `minikube` not planned.
