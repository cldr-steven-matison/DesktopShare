...
- **Reboot survival (#322, 2026-09-09):** `systemctl` units by mechanism —
  | Service group | Boot mechanism |
  |---|---|
  | k3s (`mynifi`, Kafka, Flink operators, ingress-nginx) | `k3s.service` (enabled, systemd native) |
  | EFM `minifi-java` | `minifi-java` service (SysV shim, `is-enabled=disabled` but came back on 09-08 — TBD whether durable) |
  | Docker serving tier (vLLM, TEI embed/rerank, whisper, qdrant, tei-kb) | `nvidia-serve-boot.service` (`systemd` oneshot, `After=docker.service nvidia-container-runtime.service`, GPU-gated `nvidia-smi` wait) — `serve-boot.sh` does `docker rm -f` + recreate for all 6 containers. `HF_HUB_OFFLINE=1`/`TRANSFORMERS_OFFLINE=1` baked in so vLLM never crash-loops on HF Hub egress. Why `--restart unless-stopped` failed: GPU device not ready when dockerd restart pass ran, containers failed once and gave up. `docker start` does not reattach bridge IP/ports — recreate only works. Deploy: `sudo cp files/issue-322/nvidia-serve-boot.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable --now nvidia-serve-boot` |
