...
- **2026-07-25 (same day, later) — Rule 2 (GET-then-PUT destroys sensitive properties) and `debugging.md` item 10 both gained a "this recurs across unrelated processors" note.** A prior session GET-then-PUT'd `TwitchChatListenerProcessor` and destroyed its Client Secret/Refresh Token exactly the way `XLivePostProcessor`'s creds were destroyed on 2026-07-12 — a second, independent occurrence on a completely different processor, `validationStatus` staying `VALID` both times. The rule's wording was already generic, but nothing in it flagged that this had already repeated once; now it names both incidents so it reads as a standing risk on any sensitive-property processor, not lore about one. Fixed live (Parameter Context re-hydrated from `/home/tunas/.env`, processor rebound to `#{param}` refs, bot back online) — see `streamers-twitch-bot.md` for the incident writeup. Re-synced on **MINI-Gaming-G1** as of 2026-07-25.
