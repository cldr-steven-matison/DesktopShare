...
> **Status (2026-08-27 — all three halves BUILT; swept 2026-09-16):** work-stream **H** ([#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240), closed; EPIC [#356](https://github.com/cldr-steven-matison/DesktopShare/issues/356)), feeding Ch15–Ch17 — the retrieval, validation and measurement halves of local AI for Claude Code on the box. The whole work-stream is built on `spark-dd06`. **Retrieval (§2–§3, H1–H3):** Qdrant (`:6333`) + TEI (`nomic-embed-text-v1`, 768-d, `:8080`) up, `desktopshare-kb` = **4197 chunks**, queried through the `kb_search` MCP tool (`ds-kb`). **Validator (§4, H4–H5):** `files/issue-226/kb/validator.py` passes the ten-command gate 6/6+4/4 in <1 s (reasoning off), wired into `guard.sh` as an advisory, spark-dd06-only, fail-open emitter (rule 10.5) — the one-week soak never produced a verdict because the validator writes no ledger (a pass/fail needs instrumentation first; carried in #356). **Measurement (§5):** `files/issue-226/kb/measure.py` produced the seed local-vs-hosted lint pair — ~500× cheaper and ~10× faster locally, hosted clearly higher-quality, i.e. the "hybrid" verdict with numbers (§5.1 as-built). Three box-settled unknowns from the earlier plan: the **prebuilt TEI `121-latest` (sm_121) tag runs native — no `CUDA_COMPUTE_CAP=121` build**; the **stock `mcp-server-qdrant` was superseded by the v2 FastMCP server** (unnamed TEI vector + nomic prefixes, §3.3); and the **model must run with reasoning OFF** for a sub-second verdict (§4.4). **Remaining for #240:** the H5 soak week, and the full authoring-chain §5 baseline. As-built detail is inline in §§3–5.
...
**Remote transport (added 2026-09-14, #335).** The same `kb_search` tool is served over MCP HTTP on `:8191` by `kb-remote.service` for the other array devices — design and unit in `nvidia-dgx-spark-remote-kb.md`; the stdio server above stays the on-box path.
...
### 4.4 As-built (2026-08-27) — H4 built, H5 wired

`files/issue-226/kb/validator.py` is the validator: it assembles the seven cardinal rules above as a fixed digest, grounds them with a live KB retrieval over `kind=rule/plan/completed`, and asks the box's own model for a `{verdict, rule, reason}` JSON. **The one decision the box forced: reasoning must be OFF** (`chat_template_kwargs:{"enable_thinking":false}`). This is a reasoning model — with thinking on, a verdict spends the whole `max_tokens` budget in hidden reasoning and returns `content:null` (the SparkLlmBridge gotcha), and even when it answers it takes ~25 s, far past the 3-5 s hook budget. Thinking off, a grounded verdict returns in **~0.7-0.9 s**.

**H4 gate — met.** `validator.py --selftest` runs the ten-command set (six violations from §4.1, four clean): **6/6 violations caught with the right rule cited, 4/4 clean passed, slowest 0.9 s.** The six violations were each labelled `BLOCK` with the correct rule; the four read-only commands `ALLOW`.

**H5 — wired, advisory only; the one-week soak has no verdict (no ledger).** `guard.sh` gains rule 10.5: when a Bash command matches a rule-relevant *risky* shape (a NiFi-API PUT, an `agent-deployer`/`agentIdentifier`/`generateCommand`, a `kubectl delete pod`, a `port-forward`/`minikube tunnel`, a `rollout restart` or a live-deploy delete) it calls the validator and surfaces the verdict as **context on an `allow`** — never a `deny` or `ask`. Every safety property the hook already learned is honoured: **spark-dd06 only** (a hard no-op on every other device — the risk of touching a hook that runs on the whole fleet), risky-shape-gated so the common command pays nothing, an endpoint reachability check, a hard `timeout`, fail-open on every path, and `DS_VALIDATOR=0` to disable. Verified in-session: a `kubectl delete pod mynifi-0` payload drew `[local validator · ADVISORY] BLOCK — Deleting the NiFi pod wipes the emptyDir repos and destroys the entire flow (Rule 3)`; a `git status` fell straight through. Blocking is not discussed until a clean week of advice that never stopped real work (§6 H5) — and even then, the hook's timeout-is-a-pass rule means a validator that hangs silently approves, so it stays advisory with a short fail-open timeout by design.
...
### 5.1 As-built (2026-08-27) — the seed measured pair, and what it says

`files/issue-226/kb/measure.py` runs a movable workload on the box's model, records what it actually cost, prices those *same* tokens at published rates, and pins the deterministic `doc-check.py` count next to it as the quality anchor. First workload measured: **`lint`** on `nvidia-dgx-spark-efm-agent.md` (the doc-check quality scorer already made this the sanctioned move once the §4.3 smoke test passed), run both ways — locally, and once through the hosted chain (a Sonnet agent given the identical writing-style rules + document).

| | **Local** (box's own model) | **Hosted** (Sonnet agent) |
|---|---|---|
| latency | **6.8 s** | ~77 s (agent: file reads + turns) |
| tokens | 13,211 in + 610 out = **13,821** | **53,809** (agent scaffolding included) |
| Anthropic cost | **$0** | ~$0.15–0.22 for the agent run; the *same single inference* hosted would be **$0.033 Sonnet / $0.081 Opus** |
| findings | a few, led by a **false positive** ("missing YAML front matter" — the house style requires none) | **5**, all real style-rule violations (no first-person voice, not Symptom→Diagnosis→Fix, opening frames *what* not *why*, generic headers, why-not-in-block) |
| doc-check.py (ground truth) | 2 errors, 0 warnings | (same doc) 2 errors, 0 warnings |

**What the pair actually says — the honest read, not a savings headline (§5.5).** On cost and latency the box wins by more than an order of magnitude: **~500× cheaper per inference and ~10× faster**, on its own electricity. On *quality* the hosted pass is clearly better — Sonnet's five findings are all genuine `writing-style.md` violations, while the local model's lead finding was plausible-but-wrong, exactly the failure the BridgeMind verdict (§4.1) predicted for a model at this tier. So the measured conclusion matches the §5 table's own verdicts: **move the cheap, high-volume, mechanically-checkable half (extraction, first-pass lint) to the box, keep the judgment on the hosted model** — the "Hybrid" row, now with numbers behind it. The full authoring-chain baseline (one complete document through `authoring-workflow.js`, per-phase tokens on both chains) remains the larger §5 exercise; this is the seed that proves the harness and the shape of the answer. Note the token columns are not apples-to-apples: the local number is a single inlined inference, the hosted number is a full *agent* (its own context, tool calls, turns) — which is itself the point the cost guidance makes about agent teams using ~7× the tokens of a single call.

> **Standing measurement (2026-09-02):** the one-off pair above is now the *seed* of a ledger, not the scoreboard. Work-stream L (`nvidia-dgx-spark-offload.md`, `files/issue-226/kb/offload.py`) keeps a dated record of the box-versus-Claude generation share across every session on `spark-dd06` and executes this section's table rung by rung; first reading 1.50 %, and the KB row had been invoked by a session twice. The table and verdicts here stay as written — L is what runs them.
...
| H4 ✅ | **Done 2026-08-27.** Validator built (`files/issue-226/kb/validator.py`) — cardinal-rule digest + live KB grounding + the box's model with **reasoning disabled** (`enable_thinking:false`, or a verdict costs ~25 s and returns `content:null`). `--selftest` runs the ten-command set | **Met** — 6/6 violations caught (each with the right rule cited), 4/4 clean passed, **slowest 0.9 s** |
| H5 ✅ wired / ⏳ soak | **Wired 2026-08-27.** Advisory emitter added to `guard.sh` (rule 10.5): risky-shape-gated, spark-dd06-only, endpoint-checked, hard `timeout`, fail-open, `emit_ctx` (allow + context, never a deny). Verified firing on a real risky command in-session; `git status` falls through | Gate is a **one-week soak** with no false positive that stopped real work — inherently future; blocking stays off until then |
...
- ✅ / **deferred:** the §5 before/after pair exists for one workload (lint) on one document — tokens, latency and `doc-check.py` count on both the local and hosted chain (§5.1). The *full authoring-chain* baseline (one document through all of `authoring-workflow.js` with per-phase accounting) is **not the current focus** — a still-valid future measurement, parked while the work moves to AWS field-validation. The live question the seed raised — whether a *bigger* local model could take on the authoring itself — is a model-evaluation task under work-stream A (#232) (2026-08-27).
...
- Shipped: [#240](https://github.com/cldr-steven-matison/DesktopShare/issues/240) closed 2026-09-10 with the work-stream H row in `nvidia-dgx-spark-plan.md` §4 carrying the shas; Phase 5's local-KB half of the gate is met. The §5 measurement continued as work-stream L (`nvidia-dgx-spark-offload.md`, #294, closed 2026-09-10 at a 7.97 % generation share).
...
- Companion docs: `nvidia-dgx-spark-plan.md` (the EPIC spine, §5 Phase 5) · `nvidia-dgx-spark-research.md` (§5 GitHub examples, §10 local KB and agent loops, §11 the verification table) · `nvidia-dgx-spark-landscape.md` · `nvidia-dgx-spark-runbook.md` · `nvidia-dgx-spark-cso-demos.md` · `Complete Developer Guide for Nvidia Spark with Cloudera.md` · `files/nvidia-spark-guide/README.md`
