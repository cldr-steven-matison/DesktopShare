...
## Handoff done (WindowsDesktop) — 2026-08-04/05

Step 4 landed, with one detour: the `SparkPlug` PG had actually gone missing from live `mynifi-0` entirely (a casualty of the 2026-07-10 pod-recreate incident, not just a stale doc), so it had to be re-imported from the last committed export before either leg could be wired. Both `ConsumeMQTT` and `ConsumeMQTTIIoT` got their own `PublishKafka` (`xiao_telemetry` / `sparkplug_telemetry`, both keyed `${device_id}`) — the design-note correction below turned out to matter, so both legs were wired, not just the JSON one.

Verifying the wiring turned up a second, unrelated problem: `MicroFi`'s leftover `GenerateFlowFile`/`PublishMQTT` Repro58 rig was publishing to the same `test/sensor/data` topic the real XIAO uses, at ~1/sec — `xiao_telemetry` was 100% that noise. Removed via the EFM Designer API 2026-08-05, confirmed clean (`xiao_telemetry` and raw `test/sensor/data` both silent after removal — which also means the real XIAO isn't currently powered on, separately from the fix). Full incident writeup and the wiring detail are in [`guide/ch20-sparkplug-demo.md`](guide/ch20-sparkplug-demo.md), which folds this doc's real-hardware pass into the guide.
