...
> **Ranger authz note (from #180, this Base deploy).** Integrating the **CFM NiFi operator** with this cluster's Ranger: the operator's `spec.security.ranger` is an **authorizer**, **orthogonal to authentication** and combinable with `spec.security.kerberos`. The plugin downloads policies over **mTLS** (`xasecure.policymgr.clientssl.*` from `tls.secretName`; `adminIdentity` = the Ranger **server** DN). On a Kerberized Ranger there are **two** structured download-auth paths, not a wall: **(1) mTLS** — register the plugin cert CN in `policy.download.auth.users`; **(2) SPNEGO** — also set `spec.security.kerberos`. A `401 loginId=null` on the secure policy-download endpoint means the caller CN/principal isn't registered (the #180 first probe used an unregistered host cert), not that mTLS can't authenticate. Full-enforcement runbook (handed to the Mac): `files/issue-180/RUNBOOK-mac.md`; authoritative operator doc: `files/issue-180/cfm-operator-ranger-authorization.md`.
