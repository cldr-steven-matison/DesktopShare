...
The one prerequisite was a Knox-SG entry for the box's public source IP. The SG (`srm-iceberg-knox-sg`, `sg-0938cb277fe790c79`) allowed only `179.64.39.45/32` on 443 — a **stale VPN-pool exit IP**, not a home IP; the corp VPN pool now exits on `165.1.200.x`. `165.1.200.192/32` was added to the SG on 443 for the validation window and recorded as revoked — but it was still present when #355 read the SG at 21:10 UTC the same day (its removal is an open item on #355). The corp VPN exit rotates within `165.1.200.x`, so a `/32` goes stale on reconnect — the durable fix is the corp range. And for anything running **inside a k3s pod** the relevant address is not the host's at all: the #352 policy routing sends pod egress out the home ISP, so the NiFi pod needed its own `/32` (see §3.3 below).

Still outstanding:

- **NiFi-hosted native read (§3.3 paths 2–3) — PROVEN on the box, 2026-09-16 21:02 UTC ([#355](https://github.com/cldr-steven-matison/DesktopShare/issues/355)).** The `nifi-iceberg-read-bundle` NAR was built on the box (JDK 21, user-space Maven, CFM services-api bootstrapped off the pod) and hot-loaded into `mynifi-0`; the proven mac-leg export was uploaded as-is (bucket delta only) and bound to the `iceberg-demo-params` Parameter Context. `GetIceberg` read `poc_uc2.airlines`; `QueryIceberg` pushed `ref(name="code") == "AA"` with a 4-of-5 projection; `QueryFlights` on `poc_uc2.flights WHERE flight_month='2026-03'` planned **1 data file, 11/12 manifests skipped** — the same metadata-layer pruning the Mac leg showed, now from this box's CFM operator. The earlier blockers were real and are fixed under [#352](https://github.com/cldr-steven-matison/DesktopShare/issues/352) (k3s `node-ip` re-pin, `k3s-vpn-route.timer`); the last one was networking, not NiFi: the pod egresses on the home ISP, not the VPN, so the Knox SG needed the **pod's** `/32`. Capture: [`files/issue-355/nifi-validation-2026-09-16T2102Z.txt`](files/issue-355/nifi-validation-2026-09-16T2102Z.txt); re-export [`files/issue-355/flows/IcebergRESTCatalogDemo.flow.json`](files/issue-355/flows/IcebergRESTCatalogDemo.flow.json). Path 1 (`InvokeHTTP` inside NiFi) is covered by the identical Knox-OAuth2 + `loadTable` interaction the curl proof above exercises; the `IcebergRestCatalogDemo` InvokeHTTP PG is not deployed on this box.
