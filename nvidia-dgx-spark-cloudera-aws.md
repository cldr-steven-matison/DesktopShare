...
> **Status (2026-09-09).** Work-stream **I** of the DGX Spark EPIC — [#226](https://github.com/cldr-steven-matison/DesktopShare/issues/226), issue [#241](https://github.com/cldr-steven-matison/DesktopShare/issues/241). This doc is the source for the **platform** scope: using the DGX Spark with each Cloudera platform form factor on AWS. Restructured 2026-09-09 into three explicit paths — **(1) CDP Base CE on AWS, (2) CDP Public Cloud on AWS, (3) Cloudera AWC on AWS.** Path 3 keeps its own peer doc, `nvidia-dgx-spark-cloudera-awc.md` (#283); this doc carries the reference and the field-validation checklist, not a duplicate runbook. In every path the DGX Spark is a *client*, never a cluster node, and the parity payload is the OpenAI-compatible API on both sides. Nothing here has been field-run yet; every model named is a candidate with its citation. Feeds guide chapters ch05, ch18, ch19, ch21 (Cloudera AI on AWS) and ch24 (the same-code arc); the AWC form factor is ch20 (platform) and ch22 (Cloudera AI on AWC), sourced from the peer doc.
...
- **NiFi-hosted read (§3.3 paths 1–3 inside NiFi) is blocked by a VPN↔k3s routing conflict, not by the catalog.** The box's NiFi runs on k3s and reaches its API server at the ClusterIP `10.43.0.1`; the corp full-tunnel VPN installs `default dev tun0` with **no route for the ClusterIP range `10.43.0.0/16`**, so ClusterIP traffic is swallowed by the tunnel and the NiFi node cannot complete cluster-coordinator leader election (Jetty serves 503, then the pod crash-loops). VPN up serves AWS but breaks k3s ClusterIP routing; VPN down fixes k3s but loses AWS — a mutually-exclusive posture on one box (a NiFi restart and a `k3s` restart both failed for this reason). Fix needs a split-tunnel/route carve-out (exclude `10.42.0.0/16` + `10.43.0.0/16` from `tun0`); tracked as its own infra issue [#352](https://github.com/cldr-steven-matison/DesktopShare/issues/352). So `InvokeHTTP`+OAuth2, `GetIceberg`, `QueryIceberg` were not run in NiFi this pass; the curl proof above exercises the identical Knox-OAuth2 + `loadTable` interaction.
- **Native `GetIceberg` / `QueryIceberg` (§3.3 paths 2–3)** additionally need the `nifi-iceberg-read-bundle` NAR built (source-only in `/home/tunas/NiFi2-Processor-Playground`) and deployed into the NiFi pod.
...
Until the box reaches `goes01`, path 3 stays documented-not-run; the peer doc carries the detail and this checklist is what closes it.
