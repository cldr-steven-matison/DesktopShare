...
1. **Which Windows box** — MINI-Gaming-G1 (i9-13900HK, has minikube + Tailscale, currently runs the array's cluster) or TunaStarlink (Beelink SER9, Starlink-connected, currently runs the StarlinkAI class)? Or a fresh VM for reproducibility? Both existing boxes may have prior MiNiFi state; a VM starts clean.
2. **Python currently installed** on the target — need `python --version` before running Step 5.
3. **Internal archive access** — Cloudera Slack `#minifi` or the internal Cloudera archive — do we have alternate MSI builds (e.g. bound to different Python versions, debug builds with symbols) beyond the public `1.26.02-b30`?
4. **Preserve or discard the existing agent identity?** The old `agentIdentifier` `2fcc8516-23cc-4f88-90eb-ce74e15bf36a` from the 2026-06-08 install is presumably still registered in EFM as a `WindowsDesktop` class agent. If we want the new install to look like the same agent, reuse that ID. If we want a clean slate, generate a fresh UUID.
