...
## Early-ack round-trip test — issue #55, 2026-07-31

The processor-catalog table above already established (2026-07-25) that `HandleHttpRequest`/`HandleHttpResponse`/`StandardHttpContextMap` are present. What hadn't been tested: whether the **early-ack wiring** actually works — `HandleHttpResponse` immediately after `HandleHttpRequest`, response flushed to the caller while the rest of the flow keeps running behind it — and what it costs to run. [#55](https://github.com/cldr-steven-matison/DesktopShare/issues/55) asked both, to unblock [#28](https://github.com/cldr-steven-matison/DesktopShare/issues/28)'s XIAO/MicroFi real round-trip.

**Method:** a disposable local `docker run` of the already-pulled `container.repo.cloudera.com/cloudera/nifi-minifi-java:latest` image (`1.23.04-b15` — a different build than the array's EFM-staged `2.24.08.0-19`, useful as a second, independent confirmation) with a hand-authored `config.yml`:

```
HandleHttpRequest (:8085) → HandleHttpResponse (200) → LogAttribute
```

Not run through EFM — no agent enrolled, no flow published, container removed after the test. Pure local evaluation, per issue scope.

**Results:**
- **NAR bundle confirms both processors + the controller service ship in `minifi-standard-nar`/`nifi-http-context-map-nar`** — same finding as the 2.24.08.0-19 manifest already in this doc, now cross-confirmed on a second version.
- **EFM's Designer already exposes them for a Java-class flow with zero side-loading**: `GET /efm/api/agent-manifests/ff1aaa62-...` (the manifest live-assigned to `WindowsDesktop`/`KubernetesPodJava` today) lists `HandleHttpRequest`, `HandleHttpResponse`, and `StandardHttpContextMap` in `bundles.componentManifest` — the Designer palette for either of those classes already has them.
- **The early-ack wiring works exactly as described.** `curl -X POST` got a real `200` in `~84ms`; the flowfile independently reached `LogAttribute` afterward (visible in `minifi-app.log`) — proof the response flushes without waiting on downstream processors to finish, not just before they start.
- **Cost, measured on the running container:** main JVM process RSS ≈424MB + bootstrap-watcher JVM ≈83MB ≈ **~507MB combined** (`docker stats` reported 590MB total), **493MB installed on disk** (vs the C++ agent's ~247MB disk / ~100MB RSS baseline cited in issue #55). Consistent with the `KubernetesPodJava` k8s deployment's existing `768Mi request / 1536Mi limit` sizing elsewhere in this doc — not a surprise number, just now empirically grounded. On the Jetson's 7.3GB RAM / no swap, that's roughly 7% of total RAM for the agent alone — real, but not prohibitive; still a genuine tradeoff against GPU inference headroom on that box, same caution issue #55 raised.
- **Side by side is already proven, just not on the Jetson yet.** This array already runs Java and C++ MiNiFi agents concurrently without conflict: `WindowsDesktop` (java) alongside `WindowsDesktopCpp` (cpp), and `KubernetesPodJava` alongside `KubernetesPod` (cpp) in the same cluster. Nothing about adding a Java agent replaces the Jetson's existing C++ agent (`NvidiaNano`/`NvidiaNanoAI` classes, 3 live listeners) — it would be a new, additional agent enrollment. Not done in this pass: the Jetson has no aarch64 JRE staged yet, and no Java-class MiNiFi agent has been deployed there. That's real follow-on work for #28, not part of this evaluation.

**Bottom line for #28:** the C++-only "answer must go out via Kafka" constraint (skill rule 6) is no longer a given for the XIAO/MicroFi round-trip — a Java agent on the Jetson could do a genuine synchronous request/response. Whether to actually switch the Jetson's router to Java (JVM cost vs. real round-trip) is a design decision for #28's next phase, not settled here.
...
- [x] Clear the stuck `UPDATE_PROPERTIES` retry loop on `WindowsDesktop` — **root-caused and fixed 2026-07-30**, tracked as [#38](https://github.com/cldr-steven-matison/DesktopShare/issues/38). The desired-state fact lived in Postgres `property_updates` (`agent_class='WindowsDesktop', property_key='nifi.python.command'`), also surfaced via `GET /efm/api/agent-classes/WindowsDesktop` as `customizedProperties`. EFM diffs each agent's reported properties against its agent class's `customizedProperties` on every heartbeat and re-queues `UPDATE_PROPERTIES` when it's missing — explaining 14k+ `FAILED` operation rows since 2026-07-25 and why the earlier restart only bought ~6 minutes (EFM reloads agent-class config from this same Postgres row on boot, so the next heartbeat cycle just re-triggered it). `PUT /efm/api/agent-classes/WindowsDesktop` with an empty `customizedProperties` returned 200 but silently didn't persist (confirmed via direct DB read) — no Spring cache-eviction endpoint exists either. Actual fix: `DELETE FROM property_updates WHERE agent_class='WindowsDesktop' AND property_key='nifi.python.command';` directly against the `efm` database on `ssb-postgresql` (EFM's own Postgres, `jdbc:postgresql://ssb-postgresql.cld-streaming.svc:5432/efm`), followed by `kubectl rollout restart deployment/efm -n cld-streaming` to force the JVM to reload agent-class config from the now-clean row. Verified: zero new `operation` rows for this agent in 85+ seconds (17+ heartbeat cycles) post-restart, vs. one every ~5s before; `WindowsDesktop` Java agent process confirmed still running uninterrupted since 2026-07-29 20:48 (only a transient `SocketException: Connection reset` in its log at the moment of the EFM restart, expected).
