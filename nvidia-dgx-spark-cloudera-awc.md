...
**Status (#343, 2026-09-16):** The Cloudera AI experience (`goes01-cai-c-fe629e`) serves only the UI SPA. Every path returns `200 HTML` — no REST API endpoints or swagger for inference. The UI requires manual endpoint creation via **Serving → Model Endpoints → Create Endpoint**. Once deployed, the endpoint URL pattern is:
...
Deployment takes 10–20 minutes (downloads model artifacts, requires GPU resources). A1 (OpenAI client) and A3 (Flink Agents swap) are **blocked** until a model is deployed.

The runbook this chapter drives, all `[TO-VERIFY]` until step 1 runs:
...
- **Lakehouse Engine (Trino).** PROVEN in #284 and confirmed #343 (2026-09-16): the coordinator is the CLE landing URL with `-admin` stripped; `hadoop-jwt` works as a `Bearer` token on `/v1/statement` (`SELECT 1` → `[1]`), with the catch that `X-Trino-User` must equal the token's own identity — Trino refuses impersonation. `SHOW CATALOGS` returns `["hive"] ["iceberg"] ["system"]` — data catalogs wired natively on the Integrated engine. `iceberg.ozone_test_db.sample_logs` is queryable (5 columns, 2 rows of demo test data). A NiFi flow on the box reaches it as one more Trino REST client, via the `trino_q` request shape in `cloudera-anywhere-getting-started.md`.
- **Iceberg on AWC.** Unlike CDP Public Cloud (`nvidia-dgx-spark-cloudera-aws.md` §3.3), AWC exposes **no Knox datashare `iceberg-rest/v1/` endpoint**. Iceberg on AWC = Trino-over-HMS + Ozone. `iceberg.ozone_test_db.sample_logs` was queryable via the Lakehouse Engine (#343). So the `GetIceberg`/`QueryIceberg` + `RESTCatalogService` read paths validated against `srm-iceberg` do **not** transfer directly; the AWC read path is Trino SQL through the Lakehouse Engine. This is a genuine form-factor difference the chapter must state, not paper over.
...
- **Streaming (CDF / CSA-SSB / CSM-Kafka).** CDF (`/cdf/api/v1/`, needs `hadoop-jwt` + XSRF) and SSB (`/api/v1/`) are reachable and auth-passing (#284, #343): CDF returns deployments, SSB auth passes. The CSM Kafka node (`10.80.133.150:8443`) connects and cert validates against `Cloudera AWC Internal CA` (valid until 2026-11-11), but SASL authentication is NOT proven — SASL/OAUTHBEARER and SASL/PLAIN with `hadoop-jwt` both fail. Kafka workload identity requires a proper token beyond the Knox cookie. Produce/consume `[TO-VERIFY]` — TLS OK, auth blocked.
...
- Whether an on-subnet relay (a small VPC-internal host, or the CDF Inbound-style path) is needed for Ozone and CSM Kafka — **Ozone is now answered: no relay needed, AWS V4 signing with `hadoop-jwt` works directly** (#343, 2026-09-16). **Kafka SASL still blocked** — produce/consume requires a Kafka workload identity token beyond the Knox cookie.
