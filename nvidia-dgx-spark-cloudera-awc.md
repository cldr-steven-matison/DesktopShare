...
- **Object Store (Ozone).** S3-compatible gateway. TCP `:443` OK on both gateways (`goes01-cle-int-ozone-s3` and `goes01-cde-udf-ozone-s3`); HTTP returns `403` without AWS V4 signing — XML error confirms *"Error creating s3 auth info. The request may not be signed using AWS V4 signing algorithm."* Needs `rclone` or `s3cmd` with AWS V4 signing for bucket/object operations. `[TO-VERIFY]` — TCP OK, V4 signing not yet tested.
- **Streaming (CDF / CSA-SSB / CSM-Kafka).** CDF (`/cdf/api/v1/`, needs `hadoop-jwt` + XSRF) and SSB (`/api/v1/`) are reachable and auth-passing (#284, #343): CDF returns deployments, SSB auth passes. The CSM Kafka node (`10.80.133.150:8443`) connects and cert validates against `Cloudera AWC Internal CA` (valid until 2026-11-11), but actual produce/consume needs a Knox-authenticated Kafka client (`kcat` or `kafka-console-producer`) — `[TO-VERIFY]`.
