...
**2026-08-09: `Apache NiFi MiNiFi` (retired C++ service) deleted entirely** (`sc.exe delete`, elevated) — confirmed no `minifi.exe` process. Real Java agent restarted via `run-minifi.bat`; all 6 ports (`8090`-`8095`) confirmed listening at the time. Registered Windows **Scheduled Task `StarlinkAI-MiNiFi-AutoStart`** (`AtLogOn` trigger, user `tunas`) running `run-minifi.bat`, same shape as the existing `MpvStreamLauncherListener`/`MatrixLauncherListener` tasks — the Java agent now restarts on logon/reboot.

**2026-08-11 (#136): screen/matrix consolidated from 4 ports to 1.** The `:8091`–`:8094` port-per-command shape above is superseded — those four `HandleHttpRequest`/`ExecuteStreamCommand`/`HandleHttpResponse` pipelines were merged into a single endpoint on `:8096`. `starlinkai_screen_control.py`'s `main()` moved to a uniform 3-arg (`action`/`screen`/`streamer`) dispatch to match (empty `streamer` for non-load actions), deployed live to `C:\minifi-manual\starlinkai_screen_control.py`, verified against the real endpoint (`matrix-load screen2` succeeded). Current StarlinkAI port map: `:8090` Lemonade router (5 endpoints), `:8096` consolidated screen/matrix control (`:8091`–`:8094` closed, superseded). `:8095` (isolated transcription test branch, see above) is unaffected by this change.
