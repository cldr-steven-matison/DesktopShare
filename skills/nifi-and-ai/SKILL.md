...
6. **`ListenHTTP` on MiNiFi C++ is fire-and-forget; MiNiFi Java is not.** MiNiFi C++ has no `HandleHttpRequest`/`HandleHttpResponse` pair — the caller gets an empty 200 ack, and the real reply must exit via Kafka keyed on a caller-supplied `request_id`. This is a **C++-agent-only** limitation. Confirmed (issue #55): the MiNiFi **Java** agent ships both processors and the `StandardHttpContextMap` controller service — verified directly in the agent's NAR bundle (both `1.23.04-b15` and the array's deployed `2.24.08.0-19`) and by running the actual early-ack flow (`HandleHttpRequest` → `HandleHttpResponse(200)` → rest of flow), which returned a real 200 in ~84ms while the FlowFile independently continued downstream. EFM's Flow Designer already exposes both — the array's `WindowsDesktop`/`KubernetesPodJava` agent classes carry a manifest (`minifi-java 2.24.08.0-19`) with `HandleHttpRequest`/`HandleHttpResponse`/`StandardHttpContextMap` in its component list, no side-loading needed. Cost: a Java agent runs ~500MB RSS / ~500MB installed vs the C++ agent's ~100MB RSS / ~247MB installed — real but not prohibitive on a 7.3GB-RAM box, and Java + C++ agents already run side by side in this array today (`WindowsDesktop` java class alongside `WindowsDesktopCpp`, `KubernetesPodJava` alongside `KubernetesPod`) with no conflict. Full writeup: `efm-windows-java-minifi.md`.
...
| `references/minifi-efm.md` | The edge side: staging agent binaries, EFM persistence, the deployer curl, Windows+Python, the (undocumented) EFM Flow Designer API, recovering a `KubernetesPod`-class agent whose heartbeat has gone silently dark (bare-pod restart, asset-sync race, IP instability), and a manifest-cache staleness gotcha when a processor's properties change but its name doesn't. |
