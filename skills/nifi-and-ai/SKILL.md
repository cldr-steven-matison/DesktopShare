...
6. **`ListenHTTP` on MiNiFi C++ is fire-and-forget.** MiNiFi C++ has no `HandleHttpRequest`/`HandleHttpResponse` pair — the caller gets an empty 200 ack, and the real reply must exit via Kafka keyed on a caller-supplied `request_id`. This is a **C++-agent** limitation — it is confirmed for full Java NiFi that the pair exists, but whether the **MiNiFi Java agent** ships `HandleHttpRequest`/`HandleHttpResponse` (and a `HttpContextMap` controller service to pair them) is untested here, and would change this rule. Tracked in issue #55; don't assume either way until it's settled.
