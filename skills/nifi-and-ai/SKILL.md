...
2. **Never GET-then-PUT a processor entity that has sensitive properties.** NiFi returns `"********"` for a sensitive property on GET; PUT the returned entity back and you write that literal string over the real credential, destroying it. Instead:
   ...
   - Confirmed to recur across unrelated processors, not a one-off tied to whichever one it happened to before: it destroyed X API creds on `XLivePostProcessor` (2026-07-12) and, independently, Twitch bot creds on `TwitchChatListenerProcessor` (2026-07-25). Any processor with a sensitive property descriptor is in scope, on every PUT, no matter how unrelated the edit looks — `validationStatus: VALID` gives no warning either way.
