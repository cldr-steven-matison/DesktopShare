...
| `references/minifi-efm.md` | The edge side: staging agent binaries, EFM persistence, deploying an agent via EFM's `generateCommand` (never a hand-built deployer curl), Windows+Python, the (undocumented) EFM Flow Designer API, recovering an EFM-managed agent whose heartbeat has gone silently dark (bare-pod restart, asset-sync race, IP instability), and a manifest-cache staleness gotcha when a processor's properties change but its name doesn't. |
