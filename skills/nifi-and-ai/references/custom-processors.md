...
0. **Start from the deployed source, not a local copy.** A checked-out `.py` drifts from what runs (a local copy was five versions behind the pod once, with a whole feature set missing — building on it would have regressed production). Read the live version with `GET /processors/{id}` → `component.bundle.version`, then `kubectl cp <nifi-pod>:/opt/nifi/nifi-current/python/extensions/<Name>.py ./deployed.py -c nifi`, apply the change to that file, bump the version, and replace the local copy with the result. A matched local test file can lag the same way.
...
4. **Explicitly switch every already-running instance to the new bundle version.** Dropping the new file on disk only makes NiFi *aware* of it (`GET` on the processor shows `multipleVersionsAvailable: true`, the new version appears in `GET /flow/processor-types` after ~55 s, no restart); a running instance stays pinned to its old bundle until you force it. The safe sequence, proven on a processor carrying sensitive literal properties:
   1. `PUT /processors/{id}/run-status` → `STOPPED` (this runs `onStopped`, which is where a processor flushes state such as a rotated token — never kill it another way).
   2. **Bundle-only partial PUT**: `PUT /processors/{id}` with body `{"revision": …, "component": {"id": …, "bundle": {"group": "org.apache.nifi", "artifact": "python-extensions", "version": "<new>"}}}` — no `config`/`properties` key at all, so the `"********"` mask of a sensitive property can never be written back over the real value. `validationStatus` reads INVALID for a moment, VALID once re-validated.
   3. `run-status` → `RUNNING`. About 6 s out of service in total.

   Never a full-entity GET-then-PUT here: `GET` returns every sensitive property masked and a full PUT round-trips the mask as the literal value, destroying the credential — this has happened more than once. Verify after: the new bundle version + VALID on the processor, `GET /flow/bulletin-board?sourceId=<id>` empty, the deployed file's md5 equals the source, and whatever external connection the processor holds is up (a Python processor's INFO logs may not appear in `nifi-app.log` at all — check the socket, not the log).
...
Two operational side effects to plan for: **disabling a controller service auto-stops every processor that references it** (a cache clear, a credential refresh) — read the service's `referencingComponents` first and restart all of them afterwards, not just the one you had in mind; and **provenance queries (`POST /nifi-api/provenance`) must be `DELETE`d after reading** — the API caps outstanding queries (409 at roughly a dozen). See `patterns.md` for the provenance loop.
