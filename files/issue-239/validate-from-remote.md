...
The EFM class flow on spark-dd06 fronts four inference doors on one listener, `http://192.168.1.203:8190`. Each door forwards to the serving tier on the same box and returns the upstream body unchanged, so the response shapes below are vLLM's, TEI's and whisper.cpp's. The point of this file is the "from a device that is not the DGX Spark" proof that work-stream G still owes (`nvidia-dgx-spark-efm-agent.md` §3, definition of done). Run any recipe from WindowsDesktop, StarlinkAI or the Jetson and paste the output on #239.
