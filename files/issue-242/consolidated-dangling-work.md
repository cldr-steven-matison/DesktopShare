...
- [ ] Expand the runbook narrative: full unbox → DGX OS → network → containers → k3s → roster → endpoints → hardening. The doc's own header says this is "still owed."
- [ ] Strike the stale pre-arrival `[ ]` checkboxes in §0/§1/§5 (the box is up as `spark-dd06`; the as-built block supersedes them) and drop the retired `:8888`/SGLang/DeepSeek `[ ]` items.
...
- [ ] Ch13/Ch14 — the two orphan items in §1 above.
- [ ] Editorial cleanup of the source doc — see §5.
...
- [ ] Ch16 — resolve the L2 gate (windowed KB adoption ≥ 50% and cache-create trending down over ≥3 rows); record the local-lint outcome; then prose.
- [ ] Ch17 — lock the offload-share measurement (currently a projection past row 1); then prose.
...
- [ ] Field-run each of the three platform paths on AWS (CDP Base CE, CDP Public Cloud, Cloudera AI) against the box as an external OpenAI-compatible client; replace the `# expected — verify on the box` blocks with as-built. Then prose. (Gated on the respective AWS envs — see Blocked.)
...
- [ ] Update [`nvidia-dgx-spark-plan.md`](https://github.com/cldr-steven-matison/DesktopShare/blob/main/nvidia-dgx-spark-plan.md) WS table and the tracker so the states match GitHub (A/C/D/F/H/AWC/EPIC closed; B/G/I/L open).
...
- [ ] Status block's "how it was designed / design-record" meta-narration (L7).
- [ ] Fleet-cutover history with dates — "cut over on that reasoning twice… 2026-08-14…" (L15).
- [ ] "tag-list queries all returned no JSON" research-session narration (L21).
- [ ] "The Jetson cost a whole debug cycle to a missing JRE" (L23).
- [ ] "both already true on WindowsDesktop" (L68); the "As built, 2026-08-27" session-log voice (L70) — keep the facts (install path, bootstrap values, `/efm/api/agents` 404), drop the voice.
- [ ] "Measured on this fleet 2026-08-22" per-device byte counts (L80) — keep the 167× ratio + OOM conclusion.
- [ ] "cost debug cycles on this fleet" (L86); "all learned the hard way on this fleet" (L164).
- [ ] Consolidation build-iteration history / "retrievable from StarlinkAI's flow before building" regret note (L105–116) — keep the canonical router shape.
- [ ] "the completeness critic found zero sources… first-of-its-kind" (L198).
- [ ] "the diagnostic that proved it is worth repeating" (L218).

**Trim Jetson to only deploy-relevant differences — cut:**
- [ ] Fleet cutover history (L15); JRE incident (L23); Jetson port/daemon map in the §2 opener (L90 first sentence); MobileNetV2 4.05 ms / 8× preprocessing detail (L159); `systemctl --user restart trt-infer` example (L161); Jetson p50/p95 sourcing (L170 — keep the "~117 ms agent overhead; GB10 makes the model faster not the agent" conclusion); "Jetson proved the cost" in §7 (L265); "Cluster→Jetson needed zero ufw" precedent (L218); Beelink/Jetson scrape-path list (L218, L280).
...
**Also:**
- [ ] Refresh the stale `Status (2026-08-28)` header to current; fix "#239 closed 2026-08-28" (it's open).
- [ ] Strike the stale "model lock still open" open question (L276 — lock closed 08-28) and the answered VLM/Whisper items where locked.
- [ ] Convert the "When this ships" section (L296–300) to as-built.

---

## Decisions needed from Steven (surface, not invent)

1. **H5 soak** (Ch15): pass or fail? The window elapsed with no verdict recorded.
2. **L2/L4 offload gates** (#294): accept "not met" as the finding, or keep soaking?
...
5. **EPIC #226 is closed** while ~12 chapters remain: reopen it, or let this issue be the tracking home?
