...
**Status: back up on `cso-prod-1` since 2026-09-10 ([#324](https://github.com/cldr-steven-matison/DesktopShare/issues/324)), and staying up until Steven asks for teardown — all seven fleet targets `up=1`: `efm`, `mynifi-web`, `nvidianano-minifi-metrics`, `starlinkai-minifi-metrics`, `windowsdesktopcpp-minifi-metrics`, and the two new `nvidiaspark1-*` jobs.** The DGX Spark is the seventh device on the fleet board, and the first with both a Layer-2 flow-level exporter (`:9936`) and a Layer-3 host exporter (`:9835`). Note it scrapes over **Tailscale** (`100.104.155.57`) by choice, not by necessity. The LAN address works too, as of 2026-09-10 16:31Z: `ufw` on `spark-dd06` allows the tailnet wholesale but the LAN port by port, and its allow-list never included either exporter, so `192.168.1.203:9936` and `:9835` timed out from this cluster until [`files/issue-233/ufw-nodeports.sh`](files/issue-233/ufw-nodeports.sh) opened them ([#233](https://github.com/cldr-steven-matison/DesktopShare/issues/233)). Both LAN targets re-proven from a busybox pod in `cld-streaming` after the change. Keep the `Endpoints` on the Tailscale address regardless — it is proven, it survives a LAN firewall change, and StarlinkAI already scrapes that way. Anonymous Viewer is on, via a values file (`files/issue-324/grafana-anon-values.yaml`), because `--set` mangles the nested `grafana.ini` section.

Previous status: taken down 2026-09-08 ([#312](https://github.com/cldr-steven-matison/DesktopShare/issues/312) perf pass) to reclaim ~874 Mi — `helm uninstall prometheus`, dashboard ConfigMaps removed, CRDs kept; the agent-side exporters kept serving. Before that, re-stood 2026-09-01 ([#140](https://github.com/cldr-steven-matison/DesktopShare/issues/140)) — all five fleet targets `up=1`. **The ServiceMonitor/Service/Endpoints CRs are not helm-owned and survive a `helm uninstall`**, so a re-stand is the helm install plus the dashboard ConfigMaps; only re-apply the monitors if something removed them. The 2026-08-26 prod cutover left the cluster with **no Prometheus/Grafana at all** (helm release gone; the monitoring CRDs and this doc's ServiceMonitor CRs survived, and every agent-side exporter kept serving the whole time). The re-stand is recorded below. This doc is the single working record for how edge-fleet metrics actually flow into the CSO Prometheus/Grafana stack on WindowsDesktop — what shipped, the exact wiring, every gotcha that cost a debug cycle, and what's still open. The published guide narrative lives in EdgeFlowManager [Ch19](https://github.com/cldr-steven-matison/EdgeFlowManager/blob/main/ch19-efm-and-nvidia-jetson.md) (Jetson slice) and [Ch21](https://github.com/cldr-steven-matison/EdgeFlowManager/blob/main/ch21-metrics-and-observability.md) (full three-layer story); this is the internal ops-grade version.
...
For a **Windows** scrape target the same is *not* true: Windows Defender Firewall defaults `BlockInbound` and silently drops (the 2026-07-31 Mosquitto/#52 lesson) — confirmed again on #170: the WindowsDesktop exporter needed `netsh advfirewall firewall add rule name="MiNiFi Java Metrics 9936" dir=in action=allow protocol=TCP localport=9936` (elevated) before the in-cluster scrape connected. StarlinkAI (#169) needed the same on the Beelink.

The target address is also per-device, and only an **in-cluster** test decides it: cluster→Beelink over LAN `192.168.1.245` times out (even with the firewall rule verified on all three profiles) while the Tailscale IP `100.110.253.66` answers — the opposite of the Jetson and WindowsDesktop targets, which scrape over LAN. Run the wget-from-a-busybox-pod test against both candidate addresses before writing the Endpoints; a test from the target host itself proves nothing about the cluster's path.
...
The Fleet board: **seven** per-device seconds-since-heartbeat tiles (green <120s / yellow <600s / red beyond; all seven at `w=3` since the DGX Spark joined — the seventh would not fit at `w=4`) · the all-device sawtooth graph (a healthy device saws between 0 and its heartbeat interval; a dying one just climbs) · Layer-2 host rows for the Jetson, WindowsDesktop, StarlinkAI and NvidiaSpark-1 (scrape stat, CPU/load, memory) · a Layer-1 row per remaining device (sawtooth, heartbeats/min, avg heartbeat size).

The NvidiaSpark-1 row is the only one carrying two scrape stats: `:9936` (agent health, the flow-level exporter) and `:9835` (box health, `dgx-spark-prometheus`). Both scrape panels need the `0→DOWN / 1→UP` value mapping the other devices' panels have, or they render a bare `1` — the source JSON shipped without it and it was added on merge (#324).
