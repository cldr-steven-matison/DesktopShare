...
**Status: back up on `cso-prod-1` since 2026-09-10 ([#324](https://github.com/cldr-steven-matison/DesktopShare/issues/324)), and staying up until Steven asks for teardown — all seven fleet targets `up=1`: `efm`, `mynifi-web`, `nvidianano-minifi-metrics`, `starlinkai-minifi-metrics`, `windowsdesktopcpp-minifi-metrics`, and the two new `nvidiaspark1-*` jobs.** The DGX Spark is the seventh device on the fleet board, and the first with both a Layer-2 flow-level exporter (`:9936`) and a Layer-3 host exporter (`:9835`). Note it scrapes over **Tailscale** (`100.104.155.57`), not the LAN. Over the LAN, `192.168.1.203:9936` and `:9835` both time out from this cluster (proven from a busybox pod) — the cause is `ufw` on `spark-dd06`, whose LAN allow-list never included the exporter or door ports ([#233](https://github.com/cldr-steven-matison/DesktopShare/issues/233)); the tailnet is allowed wholesale. It is **not** that the host is unreachable: `:80`, `:443`, `:8000` and `:32111` all answer over the LAN from here. Keep the `Endpoints` on the Tailscale address anyway — it is proven, it survives a LAN firewall change, and StarlinkAI already scrapes that way. Anonymous Viewer is on, via a values file (`files/issue-324/grafana-anon-values.yaml`), because `--set` mangles the nested `grafana.ini` section.
