...
- **[#169](https://github.com/cldr-steven-matison/DesktopShare/issues/169) StarlinkAI flow-level exporter** — **agent-side leg live 2026-08-14 (flow v7)**: fourth leg on `:9936 /metrics`, `ExecuteStreamCommand` running `powershell.exe -NoProfile -EncodedCommand <B64>` (PowerShell's native base64 is the Windows analog of the `echo <B64> | base64 -d | sh` wrapper). No `/proc` on Windows, so the script reads CIM — `Win32_Processor.LoadPercentage` → `minifi_java_host_cpu_percent`, `Win32_OperatingSystem` → `minifi_java_host_mem_total_kb`/`_mem_free_kb` (~1.7 s, inside the 15 s scrape; CLIXML noise is stderr-only, stdout is clean exposition). Listener binds `0.0.0.0`; local scrape proven. Flow export + proof: DesktopShare `files/issue-169/`. Beelink firewall rule added + verified same day (all three profiles, so LAN *and* the Tailscale adapter are covered); both `192.168.1.245:9936` and `100.110.253.66:9936` answer HTTP 200 from the Windows host — WSL-side curl to those same addresses fails refused (mirrored-mode hairpin artifact, not a real failure). Remaining: cluster-side Service/Endpoints/ServiceMonitor (+ `fallbackScrapeProtocol` — the responder sends no Content-Type, same as the Jetson) with the LAN-vs-Tailscale pick proven from the cluster, and the fleet-dashboard host row — WindowsDesktop side.
