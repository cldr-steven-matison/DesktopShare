...
**Corrected 2026-08-26 against the built 0.3.1 image (#231/#244):** flink-agents **0.3.1 ships no `vllm` integration**. `flink_agents/integrations/chat_models/` contains exactly **anthropic, azure, ollama, openai, tongyi** — the "vllm/watsonx" list came from newer docs, not the pinned release. So the fallback in this section *is* the path: the **OpenAI-completions** integration (`ResourceName.ChatModel.OPENAI_COMPLETIONS_CONNECTION` / `…_SETUP`) pointed at vLLM's OpenAI-compatible endpoint `http://vllm-service.default.svc.cluster.local:8000/v1`, model `Qwen/Qwen2.5-3B-Instruct`, `api_key` a non-empty placeholder (vLLM ignores it, the client requires it). Working example: [`files/cso-prod-1/flink-agents/workflow_agent_vllm_example.py`](files/cso-prod-1/flink-agents/workflow_agent_vllm_example.py). Note `extract_reasoning` is an Ollama-setup field and has no OpenAI equivalent. No JWT. The server already runs with auto tool-choice, which is what ReAct tool calls need. Whisper/embedding/Qdrant remain available for richer enrichment. ReAct agents are Flink jobs that call vLLM per record — the LLM is a side dependency, not the runtime.
