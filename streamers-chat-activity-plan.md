...
**What's pending:**
- **A real second Twitch bot identity for `TopStreamerJoiner`.** Its `JoinAndGreet` instance needs its own Twitch Developer app + device-code OAuth grant (same pattern as the existing bot — see `streamers-twitch-bot.md`). Client ID/Secret/Refresh Token are currently set to obvious `REPLACE_ME_*` placeholders (not real credentials — confirmed the export carries no leaked values, sensitive fields export as absent/null per the `nifi-and-ai` skill's documented flow-download behavior) purely to satisfy NiFi's required-property validator; `DRY_RUN=true` means they're never actually used until that's flipped off, which needs its own explicit go-ahead per §5 Phase 4's verify step regardless.
