...
The Python path is documented end to end in the `nifi-and-ai` skill's `references/custom-processors.md` and the two blog posts ([Custom Processors with CSO](https://cldr-steven-matison.github.io/blog/Custom-Processors-With-Cloudera-Streaming-Operators/), [How to AI with NiFi and Python](https://cldr-steven-matison.github.io/blog/How-to-AI-with-NiFi-and-Python/)) — read those if Python is what you want. From here down, everything is the Java/NAR path.
...
- The `nifi-and-ai` skill's `references/custom-processors.md` + the two custom-processor blog posts — the Python path this guide deliberately does *not* repeat.

## Follow-ups

- **Fold to the master guide.** When [#74](https://github.com/cldr-steven-matison/DesktopShare/issues/74) assigns a Part II chapter number, create `guide/chNN-nifi-native-processor.md` in [`EdgeFlowManager`](https://github.com/cldr-steven-matison/EdgeFlowManager) (chapters live there since 2026-08-05; DesktopShare `guide/` is a redirect stub) and add the row to `Complete Guide to Edge Flow Management.md`.
- **Blog cut** — [#155](https://github.com/cldr-steven-matison/DesktopShare/issues/155), the custom-processor-series post, drafted in `blog/`.
