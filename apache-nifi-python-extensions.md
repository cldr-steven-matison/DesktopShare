...
### How Many Processors Actually Have the `2.0.0.dev0` Dev Version?

Run this **one command** on your Mac (in the terminal, not inside any pod):

```bash
grep -l --include="*.py" '2.0.0.dev0' /extensions/ | wc -l
```

This will give you the **exact total number** of Python processor files in your mount that declare the dev version (regardless of whether they loaded in NiFi).

You can also run this version to see the actual filenames:

```bash
grep -l --include="*.py" '2.0.0.dev0' /extensions/
```

**Expected result** (based on the folder structure we saw earlier):  
Probably **8–10** files total have `2.0.0.dev0`, but only **5** of them are loading successfully right now.

Run the command and you’ll have the definitive number to close the plan.

That’s everything — clean, accurate, and matches exactly what you’re seeing in the UI.  

Once you run the grep (or if you already know the count), you can officially close the original plan. Let me know the number if you want me to log it in the final wrap-up.