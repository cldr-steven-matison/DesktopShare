...
- **For NiFi flows Steven hand-tunes:** what's running in the UI is truth. Dump the live flow before editing (`kubectl exec mynifi-0 -- gunzip -c conf/flow.json.gz | jq …`) — don't act on a doc's description of what's there. Never audit one of his PGs against the session history and offer to "fix" a cron or a processor set to match the doc; he edits those in the UI and the doc lags by design. Compare live against docs only when asked. **PGs Claude built** (`LiveStreamerAlert`, `TwitchChatBot`, `WatchlistChatJoiner`, …) are the opposite: edit them directly through the REST API (properties, rewiring, new processors) instead of writing values up for him to paste, and re-export the checked-in flow after.
...
2. **`agent/known-patterns.tsv` and this device's block in `CLAUDE-CHECKIN.md`** — the topic → docs table the guard injects at the call site, and the per-device paths, ports and quirks. The local memory dir holds only a handful of device-local facts (`incident-rules.md` §"Memories are not the instrument"); it is not a rule source.
3. **On `spark-dd06`: the `ds-kb` KB** — the `kb_search` MCP tool does semantic retrieval over this same corpus (root docs, `completed/`, `blog/`, `agent/`, the skill, EFM guide, flows, sub-repo code). A better *grep* for a prose question; not a replacement for loading the skill. Local to the box for now (#240). On the box the `kb-retrieve.sh` hook also runs every Bash `grep`/`rg` of repo prose through the index and injects the top cited sections at the call site (#294 L2) — go to the cited section instead of reading whole files.
...
The ladder comes **before** any `WebSearch`/`WebFetch` too: grep the installed library version, read the code that already uses the API, check the doc's session history for the same question. Once external research is warranted, one targeted fetch per confirmable fact; if results are thin after two tries, say so and pivot to what is confirmed (2026-07-02: "look at our memories and stop treating every chat like its your first day in our projects").

## Model, effort & context hygiene

**The canonical base session model, fleet-wide, is `claude-opus-4-8`** — it's the top model on the Mac (which has no Fable at all), so it's the common base every device can run (Steven's call, 2026-09-01, #247). **`claude-fable-5` is a heavy-lift opt-in, never the default session pin** — pinning Fable as the session model makes every task *and every inheriting sub-agent* run the heaviest, most expensive tier, which was the #247 cost complaint. **The cost lever is not the base model — it's using the cheaper tiers for the bulk of the work:** delegate retrieval/mechanical/waiting to `haiku`, moderate reasoning to `sonnet`, and drop effort to `low`/`medium` for routine runbook work, keeping `opus`/`fable` + `high` effort for the genuine reasoning only. **Check which model you are before deciding anything about cost: the session model is what every sub-agent inherits.**
...
- **When a background task gates a step that is already approved, proceed the instant the task is done.** Check the task state and continue; the completion notification is a backstop, not the trigger. A finished build with standing approval to flash means flash now. (2026-08-27, #265: a build finished with `rc=0` and the session idled 30+ minutes on a passive watcher waiting to be told, with the flash already approved.)
- **Set effort early with `/effort`, and bias it DOWN for routine work — this is the main cost lever.** Changing model or effort mid-session busts the prompt cache, so choose up front. Reserve `high` (and `xhigh`/`max`) for genuine hard coding, debugging, or multi-step design; default `low`/`medium` for mechanical runbook orchestration, status flips, doc edits, and retrieval-driven work. Don't sit a whole routine session at `high` on the top model out of habit — that is exactly the spend #247 is about.
...
- **A sub-agent's completion notification is not a result.** Before any status line mentions its task: (1) read its actual final text; (2) check it left no process running (`pgrep -af "docker build"` — an agent can quit while its background child keeps going, and a notification that arrives while the child runs is a pause, not completion); (3) verify the claimed artifacts in live state (image in the daemon, pod Running, file on disk); (4) only then write the status. If it quit early, check for orphans first, then relaunch to pick up, not restart (2026-08-25, #231: "Fable building" reported while the image did not exist and the build ran orphaned).
