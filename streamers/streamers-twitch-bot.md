...
- `!chat <streamer|off|me>` (also `!c overlay <streamer|off|me>`) — **broadcaster/moderator only** — re-points @tunastarlink's left-side overlay chat column at another channel's chat (raids/collabs); `off`/`me` return it to @tunastarlink's own chat. A non-mod using either form is ignored silently. Added in `TwitchChatListenerProcessor` `0.0.29-SNAPSHOT` (#300, 2026-09-06): the command emits an `overlay_relay` FlowFile → `RouteChatAction`'s new `overlay_relay` route → `InvokeOverlayRelay` (`POST /api/overlay/relay`) in the `ChatTriggers` PG. The relay itself (one anon Twitch IRC socket, target-swappable, → Kafka `overlay_chat_relay` + SSE) lives in `cso-operator-app` (`services/overlay_relay.py`), not NiFi. Silent swap — no chat ack. Golden-source spec: `twitch-overlay-chat-relay-plan.md`.
