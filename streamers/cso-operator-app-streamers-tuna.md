...
**⚠ Settled 2026-08-19 — HeyGen is the direction.** The local-pipeline sections below (Wav2Lip/SadTalker, no cloud calls) are the superseded design, kept as record. The "Session Log — 2026-07-17" at the bottom of this doc is the ground truth: local Wav2Lip was tried and rejected ("absolutely horrible" on flat cartoon art), the drawn mouth-flap composite was tried and rejected ("still looks crappy chopped"), and HeyGen works — avatar ID `be03a5aa65f946da8cf066a7708332cd`, working multi-line batched render code, creds in `.env.local` under `files/tuna-test/`. Piper itself was never disproven — it just drops out with the local route. Aligns with open issue #50 (HeyGen POC).
