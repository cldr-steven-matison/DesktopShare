...
| **screen2 MiNiFi agent came back in Session 0 — again** | The #307 durability gap reproduced on this reboot: `WindowsDesktop-MiNiFi-AutoStart` fired at 14:12:32 (result 0, before an interactive desktop existed) and `java` (PID 9892, owner of `:8082`) is in **SessionId 0**, so every `s2`/`screen2` load would launch mpv invisibly until it's restarted into Session 1. Found by the post-reboot check §4 asks for; recovery pending Steven's go (elevated kill of the Session-0 java, then `Start-ScheduledTask WindowsDesktop-MiNiFi-AutoStart` from the interactive session). Durable fix applied the same day: the task's logon trigger (`MSFT_TaskLogonTrigger`, user `tunas`) now carries a **`PT2M` delay** (`Set-ScheduledTask`, read back, task `Ready`) so it can't fire before the interactive desktop exists — to be confirmed by a `java` SessionId = 1 on the next reboot. |
