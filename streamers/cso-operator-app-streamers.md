...
### Session 29 (2026-09-07, WindowsDesktop) — reboot recovery: roster reconnect, wrong WiFi, docs catch-up

Host reboot. Three separate symptoms, three separate causes — none of them the app being "broken".

| Change | Details |
|---|---|
| **Roster store pinned to the hardcoded fallback after the reboot** | The app container restarted before CoreDNS/`ssb-postgresql`; `roster_store.start()` failed once with `[Errno -3] Temporary failure in name resolution` and never retried, so the Watchlist grid was empty and `/roster/rows` and the KB roster path returned `roster_store_unavailable` while Postgres was in fact healthy (DNS resolved and `:5432` was open from inside the pod minutes later). Immediate fix: `kubectl rollout restart deploy/cso-operator-app` → 18 rows loaded. |
| **Durable fix: background reconnect loop in `roster_store`** | `_connect()` now only leaves `_pool` set once the cache has loaded (a half-open pool can't shadow the fallback); on a failed first connect `start()` spawns `_reconnect_loop()` (2s→4s→8s→16s, then every 30s) which opens the pool, runs `_SCHEMA` + `_MIGRATIONS` and loads the cache when Postgres becomes reachable; `stop()` cancels it. Startup never blocks. Proven by an offline test that fails the connect 3× then succeeds (heals on attempt 4, readers return `None` → fallback meanwhile, `stop()` clean). **Deployed** with `MODULES=rag,streamers,efm bash scripts/deploy.sh` (matched the running pod), one pod `Running`, all `kubectl set env` creds survived, 18 rows loaded. **Working-tree change, not yet committed to cso-operator-app.** |
| **KB tab: `door request failed: ConnectError('All connection attempts failed')`** | Not the app. The gaming PC had come back on the **wrong WiFi**, so `192.168.1.203` (the Spark's `:32111` brain door and `:32112` card door) was unreachable from the host itself — ping 100% loss — while the pod still had internet. Back on the right network, `GET :32112/kb` from inside the pod returned 200 (79 KB). No change. |
| **Nano screen commands** | Same wrong-WiFi outage, plus the Nano itself had rebooted. Verified every hop afterwards: `InvokeNvidiaNanoScreen1`/`Matrix` RUNNING in the live flow, the Nano (`192.168.1.197`) `:8081`/`:8082` MiNiFi listeners and the `:5902` mpv launcher up and reachable from both this host and the `nifi` container of `mynifi-0`. Not fired a real `!load` (it drives a physical screen) — Steven's to confirm. |
| **screen2 MiNiFi agent came back in Session 0 — again** | The #307 durability gap reproduced on this reboot: `WindowsDesktop-MiNiFi-AutoStart` fired at 14:12:32 (result 0, before an interactive desktop existed) and `java` (PID 9892, owner of `:8082`) is in **SessionId 0**, so every `s2`/`screen2` load would launch mpv invisibly until it's restarted into Session 1. Found by the post-reboot check §4 asks for; recovery pending Steven's go (elevated kill of the Session-0 java, then `Start-ScheduledTask WindowsDesktop-MiNiFi-AutoStart` from the interactive session). Durable fix applied the same day: the task's logon trigger (`MSFT_TaskLogonTrigger`, user `tunas`) now carries a **`PT2M` delay** (`Set-ScheduledTask`, read back, task `Ready`) so it can't fire before the interactive desktop exists — to be confirmed by a `java` SessionId = 1 on the next reboot. **Second-order failure found while recovering:** killing only the Session-0 *child* java (9892) is not enough — MiNiFi runs as a bootstrap + child pair and the Session-0 **bootstrap (8832) respawned a new Session-0 child (4060) within a second**; both must go. And an `s2` fired while the Session-0 agent was live had left an invisible mpv (8156) owning the named pipe `\\.\pipe\mpv-screen2`, so once the Session-1 agent took `:8082` the next `s2` launched a *visible* mpv but the `loadfile` failed with `[Errno 13] Permission denied: '\\.\pipe\mpv-screen2'` (`C:\minifi-manual\windows_screen_control.log`) — a window with no stream. `_reap_windowless_mpv` cannot kill a SYSTEM/Session-0 mpv from the user session, so that guard does not cover this case; the Session-0 mpv has to be killed elevated. **Resolved 15:37:** after an elevated `Stop-Process` of 8832, 4060, 8156 and the empty visible mpv, only the Session-1 pair (7104 → 3116, owner of `:8082`) remained; the next `s2` logged `launched pid=10720 … handle=393522 setwindowpos=True` with no exception and the stream was on screen2 (Steven confirmed). |
| **Docs catch-up (this session)** | `streamers.md`: listener version `0.0.27`→`0.0.30` with the bump history, the `!chat`/`!c` overlay-relay table, the Announcer note, the #299 all-time-clips fallback, and the **rate-limit section rewritten** — the ladder was removed in `0.0.28` (2026-09-04) and the doc still described it as live. `streamers-twitch-bot.md`: §4 screen2 row now records the real live path (native MiNiFi Java agent → `windows_screen_control.py`) and the Session-0 failure mode + recovery from #307/#308; §16 reopen note (Kick loads announce nothing by design); TODO carries #311/#307/#309/#302/#304 and the after-every-reboot checks. `README.md`: `ChatTriggers` + `OnScreenAnnouncer` PG rows, KB tab and Announcer in the running table. |

**Still open:** the app-repo `nifi-processors/TwitchChatListenerProcessor.py` copy stopped at `0.0.28`; deployed is `0.0.30` from the local-only `nifi-custom-processors` — nothing tracks 0.0.29/0.0.30 source in git. `!load` on screen1 and `s2` after this reboot are unverified by a real command.
...
| Change | Details |
|---|---|
| **Label (`PLATFORM.COM/HANDLE`) moved from vertically centered to bottom-aligned in the bar** | `_burn_platform_overlay`'s `drawtext` `y` now `{bar_h}-th-{right_pad}` — uses `right_pad` as both the right and bottom gap for an even margin, instead of `({bar_h}-th)/2`. |
| **New tuna mascot icon burned in right before the label** | New `backend/assets/icons/tuna.png` (cut from Steven's source image, checkerboard chroma-keyed out to real alpha). Sized `1.7x font_size`, gapped `0.4x font_size`, nudged down `0.35x font_size` from bar-center — ratios tuned live against `KICK.COM/N3ON` mockups on #87 (v1→v5) before Steven said "perfect, ship that." Position is computed from the label's *actual* rendered text width (measured via a new Pillow dependency, `ImageFont.getlength()` against the same `DejaVuSans-Bold.ttf`/`font_size` drawtext uses) rather than a fixed offset — verified via real ffmpeg renders that it correctly tracks both a short label (`KICK.COM/N3ON`) and a long one (`TWITCH.TV/EXTRAEMILY`) before shipping. |
| **Platform logo re-centered in the bar** | Removed `_OVERLAY_LOGO_Y_OFFSET` (`kick:20`, `twitch:10`) — it pushed the logo down from true center (`0adb5ac`'s "logo dipped into video" was intentional at the time); Steven wanted it back to even top/bottom margin. `y` is now pure `({bar_h}-overlay_h)/2`. |
| **`_KICK_LOGINS` catalog swap** | `trainwreckstv` → `rampagejackson`, per Steven's ask on #86 (also had 4 currently-live Twitch streamers pinned onto the watchlist directly via `/watchlist/add`, matching what `LiveStreamerAlert` does automatically). |
| **Existing review-queue clips reprocessed in place** | The 3 overlay changes above only apply to newly-fetched clips (`FetchClips` skips re-downloading whenever `clip_path` already exists) — a one-off script (`kubectl cp`'d into the pod, run, then deleted) re-derived each already-queued clip fresh from its `clip_id` (Twitch: GQL mp4 lookup; Kick: the `m3u8` URL already stored on the queue record), re-trimmed, and re-burned with the fixed overlay, backing up and restoring per-clip on any failure. Run twice — the first attempt was killed mid-run after discovering the logo-center fix hadn't been redeployed yet (would have baked the still-broken logo into the queue); one clip's in-flight redownload was cleanly rolled back via its backup before redeploying and rerunning. **4 clips reprocessed successfully**: `PlayfulFitOcelotBCouch-fUJ0yf1cEhBZE0Eq`/`BraveInterestingDogRitzMitz-fIeFrgm9ZZmlySOQ`/`IgnorantSweetApe4Head-9aJiBEkvYYxcA5oh` (twitch/jynxzi), `kick_clip_01KZ2BS34ADS0250YNE4H9Z09A` (kick/n3on) — verified via a real extracted frame (centered logo, tuna tracking the label, single clean bar). **2 clips fell out of scope**: `kick_clip_01KZ2A7F8Q3VNNMVSHG4NJ1QJV` (n3on) and `PolishedSweetChickpeaPicoMause-sryocPh9csS-QqTX` (jasontheween) — Steven approved both into the pending-publish queue mid-session, moving them out of "review." The jasontheween one was explicitly restored to its pre-reprocess (old overlay, off-center logo, no tuna) state when its in-flight redownload got killed, so it's definitely un-fixed; the n3on one's exact state is unconfirmed (may have picked up the stale first-run code with tuna but the still-off-center logo, or may never have been touched) — not re-touched since altering an already-approved pending item wasn't part of what was asked. |

Commits: [`488cfbb`](https://github.com/cldr-steven-matison/cso-operator-app/commit/488cfbbb34535d2a74ce7e983b3219455218262e) (tuna icon + bottom-aligned label + kick catalog swap), [`9efda02`](https://github.com/cldr-steven-matison/cso-operator-app/commit/9efda02) (logo re-centered). Both deployed live (`MODULES=rag,streamers,efm`), NiFi flow state checked (0 active threads, `FetchClips` stopped) before each redeploy.
