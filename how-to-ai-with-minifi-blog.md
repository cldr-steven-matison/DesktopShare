...
The same agent fronts more than chat. Pointing new `ListenHTTP`/`InvokeHTTP` pairs at the other Lemonade endpoints lights up embeddings (`Qwen3-Embedding-0.6B`), reranking (`jina-reranker-v1-tiny-en`), and text-to-speech (`kokoro-v1`) — all four verified end-to-end through MiNiFi, each returning real output on the wire. The transport in this case is Tailscale: the agent reaches EFM at `efm-host-ip:10090` and Kafka at the same host's `:31623` NodePort over the tailnet, so the Beelink can sit anywhere with a network and still enroll.
...
One endpoint is not done: transcription (`Whisper-Large-v3-Turbo`, `:8084`) drops 100% of its multipart POSTs at `ListenHTTP` before they ever reach `InvokeHTTP`. It's a multipart-vs-buffer interaction I'm still chasing, and I'd rather say that plainly than ship a post claiming five working endpoints when four work. The four above are real; transcription is a known open item.
