...
**2026-09-12 — the NVMe stall, and the APST-off flag.** Two days after the proof the box began freezing 1–2.5 minutes after every boot, with `systemd-journald: Failed to write entry` on the console and the hung-task watchdog. It was not the serving tier, k3s, or any process: the kernel logged `nvme nvme0: I/O … timeout` → `reset controller` → `Device not ready; aborting reset, CSTS=0x1` — the Samsung `MZALC4T0HBL1` controller stopped answering once boot I/O quieted and never came back from a reset. Fix: `/etc/default/grub.d/nvme-apst-off.cfg` appends `nvme_core.default_ps_max_latency_us=0 pcie_aspm=off` to the kernel line (and shows a 3 s GRUB menu); the flagged boot ran clean. Evidence and the ruling-out order are on #322. Two operator aids came out of it: `serve-boot.sh` honours an off-switch — `touch ~/.nvidia-serve-boot.off` and the tier stays down at boot, `rm` it to re-arm, no sudo — and `files/issue-322/lockup-triage.sh` / `storage-triage.sh` / `nvme-facts.sh` are the read-only captures to run over `ssh … bash -s` in the short window after a boot when the box is wedging.
