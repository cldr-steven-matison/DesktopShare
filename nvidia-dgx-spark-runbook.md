...
The disk is part of reboot survival too. On 2026-09-12 the box froze minutes after every boot, seven boots in a row, and the services were not the cause. The 09-10 boot that had proved the mechanism ran 1.7 days and stopped at 06:10 UTC with nothing in the journal; each boot after it lasted 56 to 97 seconds. The signature, once you know it: the box still answers ping, `ssh` hangs or drops, and `journalctl -k` shows `nvme nvme0: I/O tag … timeout, aborting`, then `Device not ready; aborting reset, CSTS=0x1`, then `task jbd2/nvme0n1p2 blocked for more than 122 seconds` with every process in D state on `nvme0n1`. The Samsung `MZALC4T0HBL1` controller was hanging in a low-power state. Those kernel lines never reach the journal: journald is one of the blocked processes, so `journalctl -b -1` of a dead boot ends without an error and looks like a power cut. Read them live over SSH (`dmesg -w`, after `sysctl kernel.dmesg_restrict=0`) or on the serial console, before the freeze. Killing the Docker tier at T+16 s, disabling k3s, and checking disk space and the USB drive changed nothing; `nvme_core.default_ps_max_latency_us=0 pcie_aspm=off` on the kernel command line did. `files/issue-322/nvme-apst-off.cfg` is the GRUB drop-in, `install.sh` step 0 installs it, and `files/issue-322/triage-2026-09-12.md` holds the captures. If the box freezes with ping alive, read the kernel log before disabling anything. The serve-boot driver also has a kill-switch that needs no sudo: `touch ~/.nvidia-serve-boot.off` keeps the Docker tier down on the next boot, `rm` re-arms it, and `systemctl restart nvidia-serve-boot` (not `start`, which is a no-op on the already-active oneshot) brings the tier up on the same boot. `files/issue-322/lockup-triage.sh`, `storage-triage.sh` and `nvme-facts.sh` are the read-only captures to run over `ssh tunas@192.168.1.203 bash -s < …` from another device in that window.

```bash
sudo files/issue-322/install.sh               # GRUB drop-in (NVMe APST/ASPM off), install + enable the three units, move the agent to the native unit
...
journalctl -k | grep -iE 'nvme.*(timeout|reset)|blocked for more'   # empty on a healthy boot
```
