...
| **21** ✅ | Yes | Metrics & Observability | **Approved by Steven 2026-09-02 — [#140](https://github.com/cldr-steven-matison/DesktopShare/issues/140) ✓ closed.** Delivered 2026-09-01: Post-cutover re-stand on `cso-prod-1`: kube-prometheus-stack reinstalled, all five fleet targets `up=1` (EFM, NiFi mTLS-borrow, NvidiaNano, StarlinkAI-Tailscale, WindowsDesktop), both dashboards live via sidecar ConfigMaps. `efm-observability.md` synced (Layer-3 verdict, re-stand record, anonymous-Viewer + UID notes) **with screenshots in the MD**; Ch21 gains the fleet + per-agent dashboard figures and the rebuildable-in-two-moves Layer-0 note. Layer-3 verdict stands (heartbeat-transport rows = the MicroFi slice). **Humanized 2026-09-09 ([#295](https://github.com/cldr-steven-matison/DesktopShare/issues/295), EdgeFlowManager `467aff2`): em-dash 25.4→0/k, proof 12.5→1.0, colon 32.6→0.5, prov 7→0, bold-bullets 80.6→0%; every YAML/properties/command/sample-output block, both figures and all 32 fences unchanged; the cutover blockquote and the Java-endpoint investigation timeline are stated as mechanism + fix.** |
...
**Status as of 2026-09-09.** Steven's read of the 09-08 humanized batch: **Ch14 ✅, Ch19 ✅** ("looks good"), **Ch16 "seems light on content"** with NiFi MCP Server / an EFM-MiNiFi MCP Server as the asked-for additions, and "ok to proceed with the next batch". Ch16 reworked the same day (EdgeFlowManager `1e7451b`, new MCP-servers section, 1758→2389 words, in band) and back for re-review; the EFM/MiNiFi MCP server is its own issue, [#318](https://github.com/cldr-steven-matison/DesktopShare/issues/318) (`device:WindowsDesktop`, `status:todo`, #292's build shape). Second humanization batch landed: Ch03 `ac8c5f6`, Ch18 `3de5211`, Ch13 `ca67b83`, Ch21 `467aff2`, one commit each, every code block/command/figure kept, all four inside the band with provenance 0. The #137 read-through gate now rests on Ch16's re-read alone; work-stream G has 14 chapters left.
...
| 21 Metrics & Observability | C **approved 2026-09-02** ([#140](https://github.com/cldr-steven-matison/DesktopShare/issues/140) ✓) — obs stack re-stood, doc synced + screenshots embedded, Ch21 figures in. Chapter ✅. Humanized 2026-09-09 ([#295](https://github.com/cldr-steven-matison/DesktopShare/issues/295), `467aff2`). |
| all 21 (prose) | **G — humanization pass** ([#295](https://github.com/cldr-steven-matison/DesktopShare/issues/295), filed 2026-09-02): `efm-guide-humanization-plan.md` — blog-voice baseline (`files/prose-lint.py`), rewrite rubric, A/B on one Ch20 section, then the all-chapter pass in lint-score order. **A/B decided 2026-09-08: Steven picked the rules-only version blind, so `writing-style.md` + three lint thresholds (§"The blog-voice band") are the bar; no second-person rubric.** Ch14/16/19 humanized the same day (EdgeFlowManager `6ac3da1`/`84fd200`/`0a97248`); Ch14/Ch19 cleared on read 2026-09-09, Ch16 reworked for content (`1e7451b`) and awaiting re-read. **Second batch 2026-09-09:** Ch03 `ac8c5f6`, Ch18 `3de5211`, Ch13 `ca67b83`, Ch21 `467aff2`. **14 chapters remain**, 3–5 per session in the plan doc's ranked order (Ch01, Ch04, Ch10, Ch05 next; Ch12 and Ch20 last, for the residual em-dash/colon trim). Gates the EPIC close alongside the Ch16 re-read. |
