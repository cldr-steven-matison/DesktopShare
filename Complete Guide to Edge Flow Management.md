...
| **17**<br>🟡<br>[#18](https://github.com/cldr-steven-matison/DesktopShare/issues/18)✓, [#25](https://github.com/cldr-steven-matison/DesktopShare/issues/25), [#67](https://github.com/cldr-steven-matison/DesktopShare/issues/67), [#79](https://github.com/cldr-steven-matison/DesktopShare/issues/79) | **Edge-AI router case study**<br>src `beelink-starlink-efm-ai.md`<br>blog — | Yes — unified flow live | StarlinkAI | **2026-08-02, StarlinkAI (#79):** the `ListenHTTP`/Kafka design (C++, `StarlinkAI` class) is superseded by a Java agent (`StarlinkAIJava` class) running a single unified `HandleHttpRequest → InvokeHTTP → HandleHttpResponse` pass-through — one port (8090), all 5 Lemonade endpoints via path pass-through, no Kafka, no `request_id` correlation, real synchronous answers. This resolves [#25](https://github.com/cldr-steven-matison/DesktopShare/issues/25)'s transcription drop (the C++ `ListenHTTP` multipart buffer bug doesn't reproduce on `HandleHttpRequest`) by routing around it rather than fixing `ListenHTTP` itself. Root cause of the long debugging chase turned out to be a plain misconfiguration, not a platform issue: `InvokeHTTP`'s `Socket Read Timeout` was left at the 15s framework default, far too short for LLM inference — every request was silently timing out and never reaching the client. Fixed (10 min timeout), confirmed end-to-end with chat (real client, real content, real answer); embeddings/reranking/speech/transcription share the identical design but need a real-data retest pass, tracked as the remaining open item. Full detail and current flow screenshot in `beelink-starlink-efm-ai.md`. The `MINIFICPP-2243`-shaped drop class remains open on the C++ side elsewhere in the array — [#54](https://github.com/cldr-steven-matison/DesktopShare/issues/54) (`device:NvidiaNano`) — the same Java `HandleHttpRequest`/`HandleHttpResponse` approach is the candidate fix there too, not yet applied. |
