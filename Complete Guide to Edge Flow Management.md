...
| 6 | MiNiFi **custom** Python processors | `minifi-python-processors.md` | — | 🟡 ✍️ | Partial — arm64 k8s C++ leg field-validated 2026-07 (`EdgeTagger` first-class type, EFM Resource→asset dir, no drops; #6✓). x86_64 C++ leg in flight (#10); Windows leg via #4 | WindowsDesktop | [#6](https://github.com/cldr-steven-matison/DesktopShare/issues/6) ✓, [#10](https://github.com/cldr-steven-matison/DesktopShare/issues/10) | #10: run the x86_64 C++ leg the **managed** way (EFM Resources + Flow Designer publish, no local `config.yml` shortcut). Author `.py` processors loaded natively by the C++ agent (own type/properties/relationships) — **distinct from `ExecuteScript`, which is Ch5**; do not conflate the two |
