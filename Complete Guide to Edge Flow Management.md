...
| **6**<br>🟡✍️<br>[#6](https://github.com/cldr-steven-matison/DesktopShare/issues/6)✓, [#10](https://github.com/cldr-steven-matison/DesktopShare/issues/10) | **MiNiFi custom Python processors**<br>src `minifi-python-processors.md`<br>blog — | Partial | WindowsDesktop | Field-validated: arm64 k8s C++ leg 2026-07 (`EdgeTagger` first-class type, EFM Resource→asset dir, no drops; #6✓); x86_64 C++ leg in flight (#10); Windows leg via #4. #10: run the x86_64 C++ leg the **managed** way (EFM Resources + Flow Designer publish, no local `config.yml` shortcut). Author `.py` processors loaded natively by the C++ agent (own type/properties/relationships) — **distinct from `ExecuteScript`, which is Ch5**; do not conflate the two |
...
| **21**<br>🟡✍️<br>[#12](https://github.com/cldr-steven-matison/DesktopShare/issues/12)✓, [#16](https://github.com/cldr-steven-matison/DesktopShare/issues/16), [#19](https://github.com/cldr-steven-matison/DesktopShare/issues/19), [#20](https://github.com/cldr-steven-matison/DesktopShare/issues/20) | **Metrics & Observability**<br>src `efm-metrics.md`<br>blog — | Layer 1 | FTF3XR2065 (+WindowsDesktop subtasks) | Field-validated Layer 1 2026-07-29 (FTF3XR2065): EFM deployed, `ServiceMonitor` scrapes `efm-ui/10090` (NOT `metrics/9092` — that port serves empty), target green, `up{job="efm"}=1`, agent enrolled (`KubernetesPod`); open: agent native `9092` publisher → Grafana panel, `agentClass`-tagged EFM series. Chapter added (#12✓); `efm-metrics.md` built + Layer 1 field-validated (#16, in review). Stand the C++ agent's native Prometheus publisher onto a Grafana panel, then the WindowsDesktop subtasks — Prometheus/Grafana runbook on CSO (#19) + java/c++ `WindowsDesktop`-class validation (#20) |
