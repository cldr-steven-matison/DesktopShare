...
**Owner** is the `CLAUDE-CHECKIN.md` device that owns the *next action* (canonical `device:*`
labels — `FTF3XR2065`, `WindowsDesktop`, `StarlinkAI`, `NvidiaNano`, `macbook`; `any` = doc-only,
any host). It's a routing hint, not a lock — re-check the live roster before starting.
**Issues** links the GitHub-issue mailbox items that drive each chapter; `✓` marks a closed/done
issue, a bare `#n` is still open. Issues base URL:
`https://github.com/cldr-steven-matison/DesktopShare/issues/`.

## Status tracker

| Ch | Title | Source doc(s) | Blog | Status | Field-validated | Owner | Issues | Next action |
|----|-------|---------------|------|--------|-----------------|-------|--------|-------------|
| 1 | EFM on Kubernetes (incl. persistence) | `blog/efm-persistance.md` | `_posts/2026-07-15-Cloudera Edge Flow Manager on Kubernetes.md` | ✅📝 | Yes (EFM 2.3.1.0-2, minikube `cld-streaming`) | any | — | Write the basics directly into this chapter (what EFM is, how to deploy it, **and the Postgres + 2-PVC persistence setup** — folded in from the former standalone Ch2) — cross-reference the source doc, don't just link out to it |
| 2 | EFM Binaries & staging tree | `efm-binaries.md`, `efm-binaries-windows-python.md`, `efm-windows-java-minifi.md`, `efm-binaries-manual-deliver.md` | `efm-binaries-blog.md` (drafted 2026-07-29, pending images + publish) | 🟡 ✍️ | Yes (5 leaves verified 2026-07-25) | FTF3XR2065 | [#13](https://github.com/cldr-steven-matison/DesktopShare/issues/13), [#22](https://github.com/cldr-steven-matison/DesktopShare/issues/22) | Blog drafted from all 4 source docs (#13, in review); gather images/screenshots (#22), then publish to blog repo `_posts/` and flip to ✅📝. Authoring/heavy-lift for this item lives on FTF3XR2065, not WindowsDesktop |
| 3 | C++ processor catalog | `minifi-playground-cpp-processors.md` | — | 🟡 | Yes — 74 x86_64, 81 Windows MSI (2026-07-27), **79 aarch64 field-verified 2026-07 on NvidiaNano** (Kafka E2E 10/10 + ExecuteScript Python confirmed, #1✓) | any | [#1](https://github.com/cldr-steven-matison/DesktopShare/issues/1) ✓ | All three arches now field-verified — fold the catalog content into this chapter directly and cross-reference the source doc (doc-only; no hardware pass outstanding) |
| 4 | Java processor catalog | `minifi-playground-java-processors.md` | — | 🟡 | Yes (114 stock; 122 with the Kafka+scripting NAR drop-in — field-verified 2026-07-27 on both `KubernetesPodJava` and the real `WindowsDesktop` agent) | WindowsDesktop / FTF3XR2065 | — | Verify Docker `minifi-java:latest` on either local Docker/minikube host |
| 5 | ExecuteScript availability (4 paths) | `efm-executescript.md` | — | 🟡 | Yes (C++/Java/MSI/source mapped; Beelink C++ path re-confirmed via #2) | WindowsDesktop | [#2](https://github.com/cldr-steven-matison/DesktopShare/issues/2) ✓, [#11](https://github.com/cldr-steven-matison/DesktopShare/issues/11) | Fold into Part II narrative (doc-only). Open thread: #11 root-caused the EFM connectivity flakiness to a hung `kubectl port-forward` process on WindowsDesktop (Tailscale-bound `svc/efm` forward, PID 965661 — TCP accepts but never proxies data; sibling LAN-bound forward to the same service is clean) — fix is a restart of that one process, pending explicit ask. Production `StarlinkAI` agent's own Python-support confirmation (missing `minifi-python-script-extension.dll`) still separately open, StarlinkAI-side |
| 6 | MiNiFi **custom** Python processors | `minifi-python-processors.md` | — | 🟡 ✍️ | Partial — arm64 k8s C++ leg field-validated 2026-07 (`EdgeTagger` first-class type, EFM Resource→asset dir, no drops; #6✓). x86_64 k8s C++ leg field-validated 2026-07-29 via the full managed path — EFM Resources lifecycle (upload/assign/update/unassign/delete/reassign) + Flow Designer build-and-publish, zero validation errors, no drops (#10✓, status:review). Windows leg via #4; Jetson aarch64 real-HW + Java legs still open | WindowsDesktop | [#6](https://github.com/cldr-steven-matison/DesktopShare/issues/6) ✓, [#10](https://github.com/cldr-steven-matison/DesktopShare/issues/10) (status:review) | Remaining legs: Windows MSI via EFM Resources (not yet attempted — #4 used direct file placement), Jetson aarch64 on real HW, Java CEM agent. Author `.py` processors loaded natively by the C++ agent (own type/properties/relationships) — **distinct from `ExecuteScript`, which is Ch5**; do not conflate the two |
| 7 | Standalone MiNiFi C++ on K8s (no EFM) | MiNiFi Playground root scenario | — | ✅ | Yes (v1.26.02) | any | — | Fold the content into this chapter directly (not just a link) and cross-reference the source doc; keep the source doc itself updated as this plan progresses |
| 8 | MiNiFi Java setup | — (absent) | — | 🔲 | No | WindowsDesktop | [#21](https://github.com/cldr-steven-matison/DesktopShare/issues/21) | #21: build out + document examples for the `java` flavor of open-source MiNiFi to the root Playground repo (currently C++ only). On completion, spawn the "Introduce EFM into the Playground" (Ch9) task |
| 9 | Introduce EFM into the Playground | ClouderaStreamingOperators `minifi-agent-pod.yaml` | — | 🔲 | Partial (agent pod exists) | WindowsDesktop | — (spawned by #21) | Add `efm` section to root Playground |
...
| 12 | S2S: NiFi K8s → Cloudera DataFlow | `minifi-site-to-site.md` (stub) | — | 🔲 | No | FTF3XR2065 | — | CDP DataFlow (corp VPN/CDP access confirmed) |
...
| 21 | Metrics & Observability | `efm-metrics.md` | — | 🟡 ✍️ | Layer 1 field-validated 2026-07-29 (FTF3XR2065): EFM deployed, `ServiceMonitor` scrapes `efm-ui/10090` (NOT `metrics/9092` — that port serves empty), target green, `up{job="efm"}=1`. Agent enrolled (`KubernetesPod`). Open: agent native `9092` publisher → Grafana panel; `agentClass`-tagged EFM series | FTF3XR2065 (+WindowsDesktop subtasks) | [#12](https://github.com/cldr-steven-matison/DesktopShare/issues/12) ✓, [#16](https://github.com/cldr-steven-matison/DesktopShare/issues/16), [#19](https://github.com/cldr-steven-matison/DesktopShare/issues/19), [#20](https://github.com/cldr-steven-matison/DesktopShare/issues/20) | Chapter added (#12✓); `efm-metrics.md` built + Layer 1 field-validated (#16, in review). Stand the C++ agent's native Prometheus publisher onto a Grafana panel, then the WindowsDesktop subtasks — Prometheus/Grafana runbook on CSO (#19) + java/c++ `WindowsDesktop`-class validation (#20) |
