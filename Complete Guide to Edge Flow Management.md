...
| **17**<br>🟡<br>[#18](https://github.com/cldr-steven-matison/DesktopShare/issues/18), [#25](https://github.com/cldr-steven-matison/DesktopShare/issues/25) (status:review) | **Edge-AI router case study**<br>src `beelink-starlink-efm-ai.md`<br>blog — | Partial — 4/5 pairs done | StarlinkAI | **2026-07-30, WindowsDesktop:** `Buffer Size`/`Batch Size` 1→2 on speech + transcription fixed speech (real Kokoro MP3 verified landing in Kafka) but not transcription (retested twice, no request even echoes). This isolates the remaining bug to the multipart inbound path specifically, not a generic buffer-size issue — chat/embeddings/reranking/speech (4/5) now confirmed end-to-end. Every WindowsDesktop-side lever (property diff, republish, restart, buffer size) is exhausted. Handed to `device:StarlinkAI`: the transcription-only drop can only be pinpointed from StarlinkAI's own `minifi-app.log`. Next: read that log to find the dropping processor, then #18 closes and this row flips to ✅ |
