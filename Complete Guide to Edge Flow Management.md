...
| **19** 🟡 | Yes | EFM + NVIDIA Jetson use case | Re-authored C++→Java: enrollment relabeled historical, Java flow framed as the class design, metrics split (C++ validated / Java pending). Provenance-screenshot TODO **closed as not-possible** — EFM has no provenance view, per-FlowFile lineage is a NiFi feature; replaced with the boundary explanation plus the class/agent-manifest mismatch that keeps the live C++ agent from applying the class's Java flow ([#165](https://github.com/cldr-steven-matison/DesktopShare/issues/165)). Remaining: Java metrics path + Grafana panel ([#166](https://github.com/cldr-steven-matison/DesktopShare/issues/166)); firewall/scrape ([#139](https://github.com/cldr-steven-matison/DesktopShare/issues/139)). |
