...
| **9**<br>✅<br>[#29](https://github.com/cldr-steven-matison/DesktopShare/issues/29) (status:review) | **Introduce EFM into the Playground**<br>src `minifi-playground-efm-level2.md`<br>blog — | Yes | WindowsDesktop | **2026-07-30, field-verified live:** built a separate "Level 2" EFM-managed variant of both C++ and Java flavors (`PlaygroundCpp`/`PlaygroundJava` EFM classes, bare-pod agent-deployer bootstrap same shape as `KubernetesPod`/`KubernetesPodJava`) without touching any Level 1 (plain open-source) file. Both agents `ONLINE`, both flows published clean, both confirmed producing real repeating `LogAttribute` output on schedule, not just "pod Running." Flow exports: `files/efm/PlaygroundCpp.json`, `files/efm/PlaygroundJava.json`. Open: no real routing target wired yet, only a smoke flow — router destination wasn't specified in this issue's scope |
