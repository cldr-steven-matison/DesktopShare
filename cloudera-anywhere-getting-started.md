...
- This doc is the AWC **setup / getting-started** reference. How the DGX Spark *uses* AWC (Cloudera AI on AWC as an inference backend, NiFi/Flink against the Lakehouse Engine, the same-code base-URL swap) is the **using** side, in `nvidia-dgx-spark-cloudera-awc.md` and guide chapters `files/nvidia-spark-guide/ch20-cloudera-awc-on-aws.md` (platform) and `files/nvidia-spark-guide/ch22-cloudera-ai-on-awc.md` (Cloudera AI, #283).
