...
**Status: 🟢 LIVE on the box — scoped 2026-07-31; Pony Mail API live-probed 2026-08-14; reconciled 2026-09-01 onto a single host; built + running 2026-09-01/02 (see "As built" below). `ReleaseVoteWatch` polls steven@sceneserver.net every 2 min on `mynifi` (42 processors), and **all four product legs — MiNiFi C++, NiFi API, NAR Maven Plugin, core NiFi — are proven end-to-end** (mail → parse → capped k8s Job → verdict → recommendation on `release_vote_recommendations`). steven@sceneserver.net is subscribed to `dev@`; what remains is catching a **real live vote** (every sample so far was a closed vote, which correctly fails closed), the tests-on core measurement, and the optional vLLM brief. Tracking issue [#76](https://github.com/cldr-steven-matison/DesktopShare/issues/76), device:NvidiaSpark-1. Human-in-the-loop (NiFi recommends, a human casts the vote); IMAP ingestion via steven@sceneserver.net; full source builds as dedicated k8s Jobs.**
...
- **Stage 5 Telegram delivery arm ([#289](https://github.com/cldr-steven-matison/DesktopShare/issues/289) — built + deployed live on `mynifi` 2026-09-02):** `ReleaseVoteWatch` gained an **output port `telegram-out`**, fed off `BuildRecommendation` by a `ComposeTelegramText` (`ReplaceText`) that writes a one-screen human brief (subject / leg / tag / verify·build·smoke / +1·0·−1 suggestion / job name / aarch64 caveat) — a clone, so the bulletin + Kafka terminus is unchanged. It feeds the reusable root-level **`TelegramNotify` PG** (`files/issue-289/flows/build_telegram_notify.py`): `notify-in` input port → **`SendTelegram`**, a custom Python processor (`files/issue-289/processors/SendTelegram.py`) that keeps the bot token out of provenance (Telegram forces the token into the URL path, which `InvokeHTTP` would record in `invokehttp.request.url`), owns the `[NvidiaSpark-1]` prefix + 4096-char cap + JSON escaping, and posts via `sendMessage`. Credentials are the shared fleet `TOKEN`/`CHAT_ID` (Parameter Context `TelegramNotify`, set via the API from `~/.env`; WindowsDesktop delivered them 2026-09-02, [#293](https://github.com/cldr-steven-matison/DesktopShare/issues/293)). **`SendTelegram` is VALID + RUNNING, Dry Run off, and a controlled end-to-end test send was confirmed delivered to the phone** (queue drained, no failure bulletin). Still never a vote. Live exports: `files/issue-76/flows/ReleaseVoteWatch.json` (43 proc, +`telegram-out`) and `files/issue-289/flows/TelegramNotify.json` (params export null — creds set via the API from `~/.env` on any redeploy, Dry Run defaults on).
- **Design decisions locked in the build:** no `DetectDuplicate`/DistributedMapCache (mark-as-read is the mechanism; a stateful `UpdateAttribute` guard catches re-delivered copies); the k3s API token rides as a **sensitive dynamic property** on `InvokeHTTP` (`supportsSensitiveDynamicProperties` confirmed on this build — a plain header property cannot reference a sensitive parameter); TLS to the k3s API pins the cluster CA via `PEMEncodedSSLContextProvider` (`flows/k8s-ca.pem`, CA valid to 2036) — no trust-all; the poll loop-back connection carries a **4-hour FlowFile Expiration — a deliberate exception** to the 10-min default because source builds legitimately run hours (do not "fix" it back); the optional vLLM thread-brief is not yet wired.
- **Core NiFi leg LIVE (2026-09-02) — the last deferred leg, and the deferral was wrong.** Measured, not assumed: the whole `apache/nifi` reactor (708 modules, 4016 jars) verifies + compiles + packages in **22m38s** on this box at the full quota with `-T 1C -DskipTests` and an 8 GB heap — comfortably inside the flow's 4-hour poll window, not the multi-hour job the plan assumed. `RouteLegManifest` now routes three ways (cpp / core / maven) and `BuildCoreManifest` claims the whole quota with tunable `#{Core Maven Args}` / `#{Core Maven Opts}`, so tests can be switched on without regenerating the flow; while they are off the entrypoint stamps that into the verdict `note`, so a recommendation always says whether tests ran. Parse validated against a **real core vote body** pulled from the list archive (`[VOTE] Release Apache NiFi 2.11.0 (RC1)`): all five fields extract, and its `SHA512` matches the shipped 2.11.0 artifact byte-for-byte. **No leg is deferred any more** — `LogDeferredLeg` is retired.
- **Maven legs LIVE (2026-09-02):** `RouteLegManifest` splits the parsed vote into the cpp Job manifest or the Maven one (`maven-build/entrypoint.sh` in the `maven-build-entrypoint` ConfigMap + stock `maven:3.9-eclipse-temurin-21`, pulled by containerd — no image import needed). **NiFi API and NAR Maven Plugin build for real; core NiFi stays deferred** (multi-hour) to `LogDeferredLeg`. Dry-runs green on the full verify path against the *released* 2.11.0 / 2.4.0 artifacts — the sha512s parsed from the vote emails matched the shipped source-release zips byte-for-byte. Flow-path proven end-to-end 2026-09-02: re-delivered vote mails → two `rvb-java-*` Jobs dispatched by the flow → verdicts → recommendations on the topic (all runs: `files/issue-76/test/real_tag_dry_run.md`).
...
- **Re-testing a consumed mail needs TWO flags cleared** — `ConsumeIMAP` (Spring Integration underneath) marks processed messages with `\Seen` **and** the user flag `spring-integration-mail-adapter`, and its unseen-search filters on the latter. `UID STORE <uid> -FLAGS (\Seen)` alone re-delivers nothing; clear `spring-integration-mail-adapter` too (found live 2026-09-02). This is also why mark-as-read dedup survives PG re-deploys.
- **A closed vote's staging dir vanishes** — `dist/dev/nifi/<rc>` is deleted once the vote concludes, so a historical sample takes the tag-only fallback (`verifyOk:false`, Job fails, `-1/0` recommendation). That's fail-closed working, not a bug; only live votes exercise the full verify path.
