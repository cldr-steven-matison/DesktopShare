...
| Change | Details |
|---|---|
| **Screen1 (Jetson/`NvidiaNano`) unresponsive to `!load`/`!matrix`, fixed via device restart** | EFM showed the agent `ONLINE` with a heartbeat seconds old, but the device was 100% unreachable inbound — `ping` and direct `curl` to all three listener ports (8080/8081/8082) timed out, confirmed both from the gaming-PC host and from inside the `mynifi-0` pod (the actual source of the real trigger calls), ruling out a local routing fluke. That "heartbeat out fine, everything in dead" split means the box's own network stack/listeners were wedged, not a NiFi/EFM/flow-config problem. Steven restarted the device directly and confirmed it back up. |
