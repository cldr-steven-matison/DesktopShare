...
| `agent-trigger.sh <FlowName>` (added 2026-07-24, replaces `agent-liveStreamerAlert.sh`) | One-shot on-demand run of any flow via `POST /api/streamers/flows/trigger/{name}` — no client-side name validation, the backend's `TRIGGER_REQUESTS` allow-list is the only gate, so new Trigger-wired flows are usable here with zero script changes. Consolidates what used to be three separate mechanisms: `agent-liveStreamerAlert.sh`'s `PollTimer` pulse (deleted), and the informal start-then-stop hack for a single `FetchClips`/`PublishClip` run. **Not yet deployed/bot-confirmed** — see Session 21 |
...
**Still open:** real end-to-end test of all three routes (currently only plumbing-tested via `unmatched`); **backend still not deployed as of this update** — confirmed live (`curl` against the trigger endpoint still 405s, same as a nonexistent path) — `agent-trigger.sh` cannot actually work until a deploy happens; `agent-commands.md` itself (kubectl/minikube bootstrap commands, separate file) not touched; DST caveat on both crons still pending for November.
