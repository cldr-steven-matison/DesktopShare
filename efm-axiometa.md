...
- **[`axiometa-cli`](https://pypi.org/project/axiometa-cli/)** (PyPI 2.0.2, by Axiometa) — `boards`/`board`/`parts`/`part` (per-module pins, library, init code), `devices`, `provision` (private toolchain, ~7 GB), `compile`, `upload`, `monitor`, and `mcp` (six MCP tools: `list_boards`, `get_board`, `list_parts`, `get_part`, `compile`, `upload`). Installed on StarlinkAI 2026-09-10: WSL via `uv tool install axiometa-cli` (for the catalog and MCP commands) and the Windows Python 3.12 install (`py -3.12 -m pip install axiometa-cli`, which lands `Scripts\axiometa.exe`), the only side that can see the port. `axiometa devices` there lists `COM11 · ESP32-S3 (Espressif native USB)`. The catalog commands refuse without a key: `axiometa boards` → `A headless key is required. Sign in at studio.axiometa.io, open Axiometa Headless from the account menu, and run: axiometa login <key>`. `devices`, `compile`, `upload`, `monitor` work without one.
...
2. **AX22 modules on the array's existing agents.** Axiometa's [Genesis XIAO Shield](https://www.axiometa.io/products/axiometa-genesis-xiao-shield) ($9.99, sold out) exposes **two** AX22 ports for a Seeed XIAO — *"two universal AX22 ports allow modules to connect without breadboard wiring."* The schematic ([SCH_SHE0002 Rev 2, 2026-03-14](https://cdn.shopify.com/s/files/1/0966/7756/0659/files/SCH_SHE0002.pdf?v=1785787911)) settles the fit: *"Full XIAO Family is supported. Except for ESP32-C5. The Port 1 Analog doesnt work."* Port 1 is `P1_IO0/IO1/IO2` = XIAO `D7/A1/D6`, Port 2 is `P2_IO0/IO1/IO2` = `A0/D2/D3`; both ports share the XIAO's `SDA/SCL` and `MOSI/MISO/SCK`, with a 10 µF cap on each port's 3V3. So the XIAO ESP32-S3 units already running MicroFi-1/2/3 seat AX22 modules directly: modules become EFM-managed sensors on boards that are already agents, and the Genesis keeps its games. Still sold out. (Same catalog also has an Arduino UNO AX22 shield and a cheap in-stock PIXIE M1 ESP32-S3.)
