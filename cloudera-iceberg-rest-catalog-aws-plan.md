...
- **Write path — ⛔ blocked by design.** The Data Sharing model is **read-only** (Ranger auto-grants the external user Read-Only). So `PutIceberg` writing to the shared catalog via the external-user credentials is denied. NiFi's `RESTCatalogService → PutIceberg` wiring is confirmed correct; a real write needs a **Ranger write/create grant** for the identity (which is outside the read-only share model), a write-capable workload-user token, or a separate write-capable catalog. **Decision pending.**
