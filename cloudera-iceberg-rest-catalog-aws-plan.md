...
> **Status:** 🟢 REST Catalog **live & validated** (2026-08-11) on **FTF3XR2065**. Env + DataLake + Impala Data Hub deployed, `poc_uc2.airlines` seeded, REST Catalog enabled via CM API, and the 4-step OAuth flow verified end-to-end (IDBroker-vended STS creds, `client.region=us-east-2`). Phase 5 (consumer matrix): **OSS Spark (minikube K8s), AWS EMR Spark, Iceberg MCP (Impala), and NiFi query (InvokeHTTP) all ✅ validated**; **NiFi native `RESTCatalogService` ✅ ENABLED+VALID** against the live catalog (read path). Empirically proven the datashare REST catalog is **read-only** (writes fail at S3 layer; workload tokens 401) — a native `PutIceberg` *write* needs a write-capable catalog. **Pending (tomorrow):** finish NiFi write (PutIceberg + HadoopCatalogService), then **Flink/SSB**, Athena, Snowflake. Redeploy automation assigns the required env roles. Env stays up until the Friday reaper. Design confirmed against a colleague's live-run runbook. No driving issue yet.
...
| **NiFi** | On `mynifi` (`cfm-streaming`): `InvokeHTTP` query chain, **and** native write via `CdpOauth2AccessTokenProviderControllerService` → `com.cloudera.nifi.services.iceberg.RESTCatalogService` → `PutIceberg`. Isolated PG `IcebergRestCatalogDemo`. | 🟡 query ✅; write blocked (read-only share) |
...
- **Native controller service — ✅ proven.** `com.cloudera.nifi.services.iceberg.RESTCatalogService` (props: `Catalog URI` = `…/cdp-datashare-access/iceberg-rest`, `warehouse-path` = the S3 warehouse, `OAuth2 Access Token Provider` = the `StandardOauth2AccessTokenProvider`) reaches **ENABLED + VALID** against the live Cloudera REST catalog, and `PutIceberg` (`catalog-service`, `catalog-namespace=poc_uc2`, `table-name=nifi_sink`, `record-reader`=JsonTreeReader) validates + runs. This is the real native-processor + controller-service path, not just InvokeHTTP.
